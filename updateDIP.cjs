...
let c = fs.readFileSync('src/components/DataIntegrationsPanel.tsx', 'utf-8');

const regexLeftPanel = /\{?\/\*\s*LEFT:\s*API INTEG\s*\*\/\}?[\s\S]*?\{?\/\*\s*RIGHT:\s*OCR INTEG\s*\*\/\}?/;

const newLeftPanel = `{/* LEFT: API INTEG */}
...
if(c.match(regexLeftPanel)){
    c = c.replace(regexLeftPanel, newLeftPanel);
    fs.writeFileSync('src/components/DataIntegrationsPanel.tsx', c);
