...
export default function QuickNotes({ currentUser }: { currentUser: User | null }) {
  const [open, setOpen] = useState(false);
  const [notes, setNotes] = useState<{ general: string, track: string, strategy: string }>({ general: '', track: '', strategy: '' });
  const [loading, setLoading] = useState(false);
  const [saved, setSaved] = useState(false);
  const [activeTab, setActiveTab] = useState<'general' | 'track' | 'strategy'>('general');

  useEffect(() => {
    if (!currentUser) return;
    getDoc(doc(db, 'notes', currentUser.uid)).then((d) => {
      if (d.exists()) {
        const data = d.data();
        setNotes({
  ...
  const saveNotes = async () => {
    if (!currentUser) return;
    setLoading(true);
    await setDoc(doc(db, 'notes', currentUser.uid), { ...notes }, { merge: true });
    setLoading(false);
    setSaved(true);
  ...
  return (
    <div className="fixed bottom-6 right-6 w-80 md:w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl rounded-2xl p-0 flex flex-col z-50 animate-in slide-in-from-bottom-5 overflow-hidden">
      <div className="bg-slate-50 dark:bg-slate-800 p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
        ...
        <button onClick={() => setOpen(false)} className="text-slate-400 hover:text-slate-600 p-1 bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-700 rounded-full transition">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="p-4">
        <textarea
          ...
          placeholder={activeTab === 'track' ? t('Notes about track conditions, tyre wear patterns...') : activeTab === 'strategy' ? t('Saved strategy setups and fuel estimations...') : t('General setups, random thoughts...')}
        ...
        {currentUser ? (
          ...
          >
           ...
           <p className="mt-3 text-xs text-red-500 text-center font-bold">{t('Please log in')}</p>
        )}
      </div>
    </div>
  );
