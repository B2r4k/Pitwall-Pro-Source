          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="flex items-center justify-between">
              <label className="font-bold text-indigo-900 dark:text-indigo-300 flex items-center gap-2 text-sm">
                {t("Official API Token (JWT)")}{" "}
                <span className="text-[10px] font-bold opacity-80 border border-indigo-200 dark:border-indigo-700 bg-indigo-100 dark:bg-indigo-900/40 px-1.5 py-0.5 rounded uppercase">
                  {t("Direct Sync")}
                </span>
              </label>
              <button
                onClick={() => setShowHelp(!showHelp)}
                className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 transition p-1 hover:bg-white dark:hover:bg-slate-800 rounded-full"
              >
                ...
                {[
                  ...
                  >
                    <input
                      ...
                      checked={
                        settings.apiModules
                          ? settings.apiModules[mod.id] !== false
                          : true
                      }
                      onChange={(e) => {
                        const newMods = {
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  setLocalSettings({ ...settings, gproToken: e.target.value })
                }
                onClick={() => {
            ...
            <div className="mt-3 p-3 bg-white dark:bg-slate-800/50 rounded-lg border border-indigo-100 dark:border-indigo-800/40">
              <div className="flex items-center justify-between mb-3 border-b border-slate-100 dark:border-slate-700/50 pb-2">
                <div>
                  <div className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                    ...
                    <Tooltip
                      content={t(
                        "Fetches updated data automatically when you open the app.",
                      )}
                    />
                  </div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    checked={settings.autoApiSync}
                    onChange={(e) =>
                      setLocalSettings({
                        ...settings,
                        autoApiSync: e.target.checked,
                      })
                    }
                  />
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(
                    ["driver", "car", "track", "weather", "risks"] as const
                  ).map((f) => (
                    <label
                      key={f}
                      className={
                        "flex items-center justify-center gap-1.5 p-1.5 border rounded cursor-pointer transition text-[10px] font-bold uppercase " +
                        (settings.apiSyncOptions?.[f] !== false
                    ...
                    >
                      <input
                        type="checkbox"
                        className="sr-only"
                        checked={settings.apiSyncOptions?.[f] !== false}
                        onChange={(e) =>
                          setLocalSettings({
                            ...settings,
                            apiSyncOptions: {
                              ...(settings.apiSyncOptions || {}),
                              [f]: e.target.checked,
                            },
                          })
                        }
                ...
                <label className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100 dark:border-slate-700/50 cursor-pointer group">
                  <span className="text-[10px] font-medium text-slate-600 dark:text-slate-400 group-hover:text-indigo-600 transition">
                    {t("Ignore filters on Auto Sync")}{" "}
                    <Tooltip
                      content={t(
                  ...
                  <input
                    ...
                    checked={
                      settings.apiSyncOptions?.ignoreFiltersAuto !== false
                    }
                    onChange={(e) =>
                      setLocalSettings({
                        ...settings,
                        apiSyncOptions: {
                          ...(settings.apiSyncOptions || {}),
                          ignoreFiltersAuto: e.target.checked,
                        },
                      })
                    }
                  />
            ...
            {apiPreview && (
              <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border dark:border-indigo-800/30 border-indigo-200 shadow-sm animate-in zoom-in-95 mt-2">
                <div className="text-[10px] font-black tracking-wider text-indigo-500 uppercase mb-2 border-b pb-1 dark:border-slate-700">
                  {t("Review & Confirm")}
                </div>

                {apiPreview.message && (
                  <div className="text-xs font-medium text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400 p-2 rounded mb-3">
                    {apiPreview.message}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {apiPreview.driver && (
                    <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 rounded text-[10px] font-bold border border-blue-200 dark:border-blue-800">
                      Driver ✓
                    </span>
                  )}
                  {apiPreview.car && (
                    <span className="px-1.5 py-0.5 bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300 rounded text-[10px] font-bold border border-orange-200 dark:border-orange-800">
                      Car ✓
                    </span>
                  )}
                  {apiPreview.track && (
                    <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 rounded text-[10px] font-bold border border-emerald-200 dark:border-emerald-800">
                      Track ✓
                    </span>
                  )}
                  {apiPreview.weather && (
                    <span className="px-1.5 py-0.5 bg-cyan-50 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300 rounded text-[10px] font-bold border border-cyan-200 dark:border-cyan-800">
                      Weather ✓
                    </span>
                  )}
                  {apiPreview.risks && (
                    <span className="px-1.5 py-0.5 bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300 rounded text-[10px] font-bold border border-red-200 dark:border-red-800">
                      Risks ✓
                    </span>
                  )}
                  {apiPreview.staff && (
                    <span className="px-1.5 py-0.5 bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 rounded text-[10px] font-bold border border-purple-200 dark:border-purple-800">
                      Staff ✓
                    </span>
                  )}
                  {apiPreview.manager && (
                    <span className="px-1.5 py-0.5 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 rounded text-[10px] font-bold border border-yellow-200 dark:border-yellow-800">
                      Manager & Economy ✓
                    </span>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-1.5 bg-emerald-500 text-white rounded text-xs font-bold hover:bg-emerald-600 transition flex items-center justify-center gap-1"
                  >
                    <CheckSquare className="w-3.5 h-3.5" /> {t("Confirm")}
                  </button>
                  <button
                    onClick={() => {
                      setApiPreview(null);
                    }}
                    className="py-1.5 px-3 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded text-xs font-bold hover:bg-slate-300 transition flex items-center justify-center gap-1"
                  >
                    <XCircle className="w-3.5 h-3.5" /> {t("Close")}
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="p-5 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30 rounded-xl relative overflow-hidden transition-all duration-300">
          <div className="flex items-center gap-2 mb-2">
            <label className="font-bold text-emerald-900 dark:text-emerald-300 flex items-center gap-2 text-sm">
              {t("Smart Screenshot Reader")}{" "}
              <span className="text-[10px] font-bold opacity-80 border border-emerald-200 dark:border-emerald-700 bg-emerald-100 dark:bg-emerald-900/40 px-1.5 py-0.5 rounded uppercase text-emerald-700 dark:text-emerald-300">
                {t("Prototype")}
              </span>
            </label>
          </div>
          ...
          <div className="flex flex-col gap-2 items-start mt-4">
            ...
            >
              ...
              {isOCRProcessing
                ? t("Scanning, please wait...")
                : t("Select & Read Screenshot")}
            </button>
          ...
          {ocrPreview && (
            <div className="mt-4 p-3 bg-white dark:bg-slate-800 rounded-lg border dark:border-emerald-800/30 border-emerald-200 shadow-sm animate-in fade-in slide-in-from-top-2">
              <h4 className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-2 border-b dark:border-slate-700 border-emerald-50 pb-1">
                {t("Detected Data")}
              </h4>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs mb-3">
                {ocrPreview.tempBase !== undefined && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">
                      Min Temp:
                    </span>{" "}
                    <strong className="text-slate-800 dark:text-slate-200">
                      {ocrPreview.tempBase}°C
                    </strong>
                  </div>
                )}
                {ocrPreview.tempMax !== undefined && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">
                      Max Temp:
                    </span>{" "}
                    <strong className="text-slate-800 dark:text-slate-200">
                      {ocrPreview.tempMax}°C
                    </strong>
                  </div>
                )}
                ...
                {ocrPreview.riskAggression !== undefined && (
                  <div className="flex justify-between items-center">
                    ...
                    <strong className="text-slate-800 dark:text-slate-200">
                      {ocrPreview.riskAggression}
                    </strong>
                  </div>
                )}
                {ocrPreview.driverWeight !== undefined && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">
                      Weight:
                    </span>{" "}
                    <strong className="text-slate-800 dark:text-slate-200">
                      {ocrPreview.driverWeight} kg
                    </strong>
                  </div>
                )}
              </div>
              ...
              {ocrPreview.rainProps && (
                <div className="mb-3 bg-slate-50 dark:bg-slate-900 rounded p-1.5 text-[10px] border dark:border-slate-700 border-slate-100">
                  <div className="font-semibold text-slate-600 dark:text-slate-400 mb-0.5">
                    {t("Detected Rain Probabilities (%)")}
                  </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded text-xs font-bold transition flex items-center justify-center gap-1"
                >
                ...
                <button
                  ...
                  className="py-1.5 px-3 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 text-slate-700 dark:text-slate-300 rounded text-xs font-bold transition flex items-center justify-center"
                >
