...
export default function AdminPanel({ onBack }: { onBack: () => void }) {
  ...
  const [dateDetectMethod, setDateDetectMethod] = useState<{
    format: string;
    sure: boolean;
  } | null>(null);
  ...
  useEffect(() => {
    const loadData = async (user) => {
      if (user?.email !== "burakis204@gmail.com") {
        setLoading(false);
        return;
      }
      try {
        ...
        const telData = snapTel.docs.map((doc) => ({
          id: doc.id,
          ...(doc.data() as any),
        }));
        telData.sort((a, b) => {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const listMatch = line.match(
          /^(?:\s*(?:[tT]|1[0-7]|[1-9])\s*[\.\,\|:]\s*|test\s*track\s*:\s*|T\s*\|\s*)/i,
        );
        ...
        if (listMatch) {
          ...
          if (!trackNameStr) {
            for (const t of TRACK_DATABASE) {
              if (
                lowerLine.includes(t.name.toLowerCase()) ||
                lowerLine.includes(t.id.toLowerCase())
              ) {
                trackNameStr = line.trim();
                break;
              }
        ...
        if (trackNameStr) {
          ...
          if (
            lowerLine.includes("first race is") ||
            lowerLine.includes("thank you")
          )
            continue;

          // Clean up trackNameStr: remove dates like "12 Haz, 2026" or "12/04/2026"
          trackNameStr = trackNameStr
            .replace(
              /\b([1-9]|[12]\d|3[01])\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|oca|ocak|şub|sub|subat|şubat|nis|nisan|haz|haziran|tem|temmuz|ağu|agu|agustos|ağustos|eyl|eylul|eylül|eki|ekim|kas|kasim|kasım|ara|aralik|aralık).*$/i,
              "",
            )
            .trim();
          trackNameStr = trackNameStr
          ...
          const matchedTrack = TRACK_DATABASE.find((t) =>
            trackNameStr
              .toLowerCase()
              .replace(/[^a-z0-9]/g, "")
              .includes(t.name.toLowerCase().replace(/[^a-z0-9]/g, "")),
          );
            ...
            if (!foundTracks.find((ft) => ft.name === fName)) {
              foundTracks.push({
                ...
                trackId: matchedTrack
                  ? matchedTrack.id
                  : "custom_" + trackNameStr.replace(/\s+/g, "_").toLowerCase(),
      ...
      if (foundTracks.length > 0) {
        ...
        const newCalendar = foundTracks.map((tr, idx) => {
          ...
          if (idx > 0) {
            ...
            currentDate = new Date(
              currentDate.getTime() + daysToAdd * 86400000,
            );
          }
        ...
        setOcrLog(
          `Found ${newCalendar.length} tracks. Extrapolated GPRO dates (Tue/Fri) from ${newCalendar[0].date}.`,
        );
      } else {
  ...
  if (currentUserEmail !== "burakis204@gmail.com") {
    return (
      <div className="p-8 text-center text-slate-500">
        Yetkisiz erişim veya kullanıcı bilgisi bekleniyor...
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-sm text-slate-500 font-bold mb-1 flex items-center gap-1.5">
              <MessageSquare className="w-4 h-4" /> Total Feedbacks
            </p>
            <h3 className="text-4xl justify-end font-black text-slate-800 dark:text-slate-100">
              {totalFeedbacks}
            </h3>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-4xl justify-end font-black text-yellow-600 dark:text-yellow-500">
              {avgRating}
            </h3>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <h3 className="text-lg font-bold mb-4 text-slate-800 dark:text-slate-200 flex items-center gap-2">
          <Calendar className="w-5 h-5 text-indigo-500" /> Global Calendar
          Manager
        </h3>
        <p className="text-sm text-slate-500 mb-4">
          Update the F1 calendar stored in the Cloud. The game uses this JSON
          format to detect the active race. You can upload an image of the track
          list to auto-extract the text into JSON structure.
        </p>
        ...
        <div className="mb-4">
          ...
          >
            ...
            {isOcrProcessing
              ? "Scanning Image..."
              : "Upload Calendar Image (OCR)"}
          </button>
          {ocrLog && (
            <p className="text-xs text-indigo-500 font-medium mt-2">{ocrLog}</p>
          )}

          {dateDetectMethod && (
            <div className="mt-2 flex items-center gap-2 text-xs font-mono font-bold">
              <span
                className={`flex items-center gap-1 ${dateDetectMethod.sure ? "text-emerald-500" : "text-red-500"}`}
              >
                <Info className="w-4 h-4" />
                Date Detection: {dateDetectMethod.format}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <h3 className="text-lg font-bold mb-6 text-slate-800 dark:text-slate-200 flex items-center gap-2">
          <Clock className="w-5 h-5" /> Telemetry Contributions
        </h3>
        {loading ? (
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-center mb-4">
              <button
                onClick={() =>
                  setCurrentTelemetryIdx(Math.max(0, currentTelemetryIdx - 1))
                }
                disabled={currentTelemetryIdx === 0}
                className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded font-bold disabled:opacity-30"
              >
                Önceki
              </button>
              <span className="text-sm font-bold text-slate-500">
                {currentTelemetryIdx + 1} / {telemetries.length}
              </span>
              <button
                onClick={() =>
                  setCurrentTelemetryIdx(
                    Math.min(telemetries.length - 1, currentTelemetryIdx + 1),
                  )
                }
                disabled={currentTelemetryIdx === telemetries.length - 1}
                className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded font-bold disabled:opacity-30"
              >
                Sonraki
              </button>
            ...
            {telemetries[currentTelemetryIdx] &&
              (() => {
                ...
                const failedStr =
                  Object.entries(tel.failedChecks || {})
                  ...
                  <div className="border border-indigo-100 dark:border-indigo-900 bg-indigo-50/50 dark:bg-slate-800/50 p-4 rounded-xl relative">
                    <div className="absolute top-4 right-4 flex gap-2">
                      <button
                        ...
                        className="text-xs bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 px-2 py-1 rounded text-slate-600 dark:text-slate-300 font-bold transition"
                      >
                        Kopyala
                      </button>
                      <button
                        onClick={() => setExpandedTelemetry(!expandedTelemetry)}
                        className="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-2 py-1 rounded font-bold transition"
                      >
                        {expandedTelemetry ? "Küçült" : "Büyüt"}
                      </button>
                    </div>
                    <p className="text-xs text-slate-500 font-mono mb-2">
                      {new Date(tel.timestamp).toLocaleString()} | Track:{" "}
                      {tel.trackId}
                    </p>
                    ...
                    {expandedTelemetry ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-4">
                        <div className="bg-white dark:bg-slate-900 p-3 rounded border border-slate-200 dark:border-slate-700">
                          <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 flex items-center gap-1">
                            <HelpCircle className="w-3 h-3" /> Hata Oranı / Bil.
                            Değ.
                          </p>
                          <p className="text-lg font-mono font-black text-indigo-600 dark:text-indigo-400">
                            {(tel.errorRate || tel.unknownFactor || 0).toFixed(
                              3,
                            )}
                          </p>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-3 rounded border border-slate-200 dark:border-slate-700">
                          <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 text-right">
                            Laps / Temp Match
                          </p>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <h3 className="text-lg font-bold mb-6 text-slate-800 dark:text-slate-200 flex items-center gap-2">
          <Clock className="w-5 h-5" /> Recent Feedback
        </h3>
        {loading ? (
          <div className="text-center py-8 text-slate-500 dark:text-slate-400">
            Loading feedbacks...
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {feedbacks.map((fb) => (
              <div
                key={fb.id}
                className="bg-slate-50 dark:bg-slate-800/50 p-5 border border-slate-200 dark:border-slate-700 rounded-xl relative group"
              >
                <span className="absolute top-4 right-4 text-xs font-mono text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition">
                  {fb.timestamp?.toDate
                    ? typeof fb.timestamp.toDate === "function"
                ...
                <div className="mb-3 flex items-center gap-2 text-sm text-yellow-500 font-bold">
                  <div className="flex gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`w-3.5 h-3.5 ${i < (fb.rating || 0) ? "fill-current" : "text-slate-300 dark:text-slate-600"}`}
                      />
                    ))}
                  </div>
                ...
                {fb.email && (
                  <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700/50 text-xs text-indigo-500 dark:text-indigo-400 font-mono inline-flex px-3 rounded-full bg-indigo-50 dark:bg-indigo-900/30 items-center justify-center py-1.5">
                    {fb.email}
                  </div>
                )}
              </div>
