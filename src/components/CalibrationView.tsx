...
export default function CalibrationView({ player, setPlayer }: Props) {
    ...
    if (player.testingData && player.testingData.length > 0) {
      const apiStints = player.testingData.map((lap, i) => {
        ...
        if (lap.tyresCond !== undefined)
          wearConsumed = 100 - parseFloat(lap.tyresCond);

        let fuelConsumed = 60;
        if (lap.fuelCond !== undefined)
          fuelConsumed = 100 - parseFloat(lap.fuelCond);
      ...
      alert(
        t("No testing/practice data found. Run Auto Sync in Data tab first."),
      );
    }
  };
  ...
  const saveToCloud = async () => {
    ...
    try {
      ...
      if (auth.currentUser) {
        await setDoc(
          doc(db, "calibration_saves", auth.currentUser.uid),
          payload,
        );
      } else {
  ...
  const handleCalculate = () => {
    ...
    const estimatedBaseFuel = calculateBaseFuel(
      trackId,
      totalFuelConsumed,
      totalLaps,
      TRACK_DATABASE,
    ...
    stints.forEach((stint) => {
      ...
      const stintWearMult = calculateBaseWearMultiplier(
        trackId,
        stint.compound,
        stint.wearConsumed,
        stint.laps,
    ...
    if (avgWearMult > 1.15) {
      ...
      newFeedback.push(
        t("✨ Excellent tyre economy. Wear is well below average."),
      );
    } else {
    ...
    if (rainEnabled) {
      const avgRainProb =
        (rainProps.q2_r1 + rainProps.r2 + rainProps.r3 + rainProps.r4) / 4;
      if (avgRainProb > 30 && stints.every((s) => s.compound !== "Rain")) {
        ...
        newFeedback.push(
          t("🌦️ Your tyre selection in transition periods looks reasonable."),
        );
      }
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 border-b border-slate-100 dark:border-slate-800 pb-4 gap-4">
          <div>
            <h2 className="text-lg font-semibold flex items-center gap-2 text-indigo-900 dark:text-indigo-400">
              ...
              {t("Analytical Calibration Suite")}
            </h2>
          ...
          <div className="flex items-center gap-2 sm:self-auto self-end">
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 dark:hover:bg-amber-900/50 text-amber-700 dark:text-amber-400 rounded-lg text-[11px] font-bold transition flex items-center gap-1.5 border border-amber-200 dark:border-amber-800/30"
            >
              <RotateCcw className="w-3.5 h-3.5" /> Auto Fill
            </button>
            <button
              onClick={loadFromCloud}
              disabled={isSyncing}
              className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-[11px] font-bold transition flex items-center gap-1.5 border border-slate-200 dark:border-slate-700"
            >
              <CloudCog className="w-3.5 h-3.5" /> Load
            </button>
            <button
              onClick={saveToCloud}
              disabled={isSyncing}
              className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400 rounded-lg text-[11px] font-bold transition flex items-center gap-1.5 border border-indigo-200 dark:border-indigo-800/30"
            >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className={`w-full p-2.5 rounded-lg border dark:border-slate-700 font-bold text-sm transition-colors flex items-center justify-center gap-2 ${rainEnabled ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-slate-50 dark:bg-slate-800/50 text-slate-400 border-slate-200 dark:border-slate-700"} `}
                >
                  <CloudRain className="w-4 h-4" />{" "}
                  {rainEnabled ? t("Active") : t("Dry Race")}
                </button>
              </div>
            </div>
            ...
            {rainEnabled && (
              <div className="p-4 bg-blue-50/50 rounded-xl border dark:border-slate-700 border-blue-100 space-y-2 animate-in fade-in">
                <h3 className="font-semibold text-slate-700 dark:text-slate-300 text-xs uppercase flex items-center gap-1">
                  <CloudRain className="w-3.5 h-3.5 text-blue-500" />{" "}
                  {t("Estimated Rain Probabilities (%)")}{" "}
                  <Tooltip content="Approximate rain chances during that session. yaklaşık olarak girin ki performans tespiti kusursuzlaşsın." />
                </h3>
                <div className="flex gap-2 text-xs">
                  {["q2_r1", "r2", "r3", "r4"].map((k) => (
                    <input
                      ...
                      max="100"
                      placeholder={k}
                      className="text-slate-800 dark:text-slate-100 w-full p-1.5 rounded bg-white dark:bg-slate-900 border dark:border-slate-700 border-slate-200 dark:border-slate-700 text-center"
                      value={(rainProps as any)[k]}
                      onChange={(e) =>
                        setRainProps({
                          ...rainProps,
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border dark:border-slate-700 border-slate-200 dark:border-slate-700 space-y-4">
              ...
              <div className="space-y-3">
                {stints.map((stint, index) => {
                  ...
                  let displayFuel = "---";
                  if (isLast && index > 0) {
                    const pastLaps = stints
                      .slice(0, index)
                      .reduce((acc, s) => acc + s.laps, 0);
                    const pastFuel = stints
                      .slice(0, index)
                      .reduce((acc, s) => acc + (s.fuelConsumed || 0), 0);
                  ...
                  return (
                    <div
                      key={stint.id}
                      className="p-3 bg-white dark:bg-slate-900 border dark:border-slate-700 border-slate-200 dark:border-slate-700 rounded-lg relative group"
                    >
                      <div className="absolute -left-2 -top-2 w-5 h-5 bg-indigo-500 text-white rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm">
                        {index + 1}
                      </div>
                      {stints.length > 1 && (
                        <button
                          onClick={() => removeStint(stint.id)}
                          className="absolute -top-2 -right-2 w-5 h-5 bg-red-100 hover:bg-red-500 text-red-600 hover:text-white transition-colors rounded-full flex items-center justify-center"
                        >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateStint(stint.id, "compound", e.target.value)
                            }
                            className="text-slate-800 dark:text-slate-100 w-full p-1.5 text-sm bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 border-slate-200 dark:border-slate-700 rounded"
                          >
                            ...
                            <button
                              onClick={() =>
                                updateStint(
                                  stint.id,
                                  "laps",
                                  Math.max(1, stint.laps - 1),
                                )
                              }
                              className="p-1.5 h-full bg-white dark:bg-slate-700 border border-r-0 border-slate-200 dark:border-slate-600 rounded-l text-slate-500 hover:text-indigo-500 transition"
                            >
                              -
                            </button>
                            <input
                              ...
                              className="text-slate-800 dark:text-slate-100 w-full min-w-[30px] p-1.5 text-sm bg-slate-50 dark:bg-slate-800/50 border-t border-b border-l-0 border-r-0 border-slate-200 dark:border-slate-600 text-center focus:ring-0 focus:outline-none appearance-none"
                            />
                            <button
                              onClick={() =>
                                updateStint(stint.id, "laps", stint.laps + 1)
                              }
                              className="p-1.5 h-full bg-white dark:bg-slate-700 border border-l-0 border-slate-200 dark:border-slate-600 rounded-r text-slate-500 hover:text-indigo-500 transition"
                            >
                              +
                            </button>
                          </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="text-slate-800 dark:text-slate-100 w-full p-1.5 text-sm bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 border-slate-200 dark:border-slate-700 rounded text-center"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wider flex items-center justify-center gap-1">
                            ...
                            {isLast && index > 0 && (
                              <Tooltip content="Last stint fuel is auto-estimated based on previous average." />
                            )}
                          </label>
                          {isLast && index > 0 ? (
                            <div
                              className="w-full p-1.5 text-sm bg-indigo-50/50 text-indigo-400 border dark:border-slate-700 border-indigo-100 rounded text-center font-mono cursor-not-allowed"
                              title={t("Estimated Average")}
                            >
                              {displayFuel}
                            </div>
                          ) : (
                            <input
                              ...
                              className="text-slate-800 dark:text-slate-100 w-full p-1.5 text-sm bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 border-indigo-200 rounded text-center text-indigo-700 font-bold"
                            />
                          )}
                        </div>
                      </div>
                ...
                <button
                  onClick={addStint}
                  className="w-full py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-indigo-600 font-bold text-sm rounded-lg border dark:border-slate-700 border-slate-200 dark:border-slate-700 border-dashed flex items-center justify-center gap-1 transition-colors"
                >
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border dark:border-slate-700 border-slate-200 dark:border-slate-700 space-y-4">
              <h3 className="font-semibold text-slate-700 dark:text-slate-300">
                {t("Overall Race Conditions")}
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1 flex items-center">
                    {t("Avg Temp (°C)")}{" "}
                    <Tooltip content="Average race or track temperature." />
                  </label>
                  <input
                    type="number"
                    value={avgTemp}
                    onChange={(e) => setAvgTemp(Number(e.target.value))}
                    className="w-full p-2 rounded-lg border dark:border-slate-700 border-slate-300"
                  />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="text-sm text-slate-400 mb-1 flex items-center gap-1">
                      {t("Fuel Burn")}{" "}
                      <Tooltip content="Fuel burn per lap. Used to update global settings after calibration." />
                    </div>
                    <div className="text-2xl font-mono text-slate-800 dark:text-white">
                      {fuelResult !== null ? fuelResult.toFixed(3) : "---"}{" "}
                      <span className="text-sm text-slate-400">L/Lap</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="text-sm text-slate-400 mb-1 flex items-center gap-1">
                      {t("Wear Factor")}{" "}
                      <Tooltip content="Your car and driver's standard tyre wear rate scale. 1.0 is default." />
                    </div>
                    <div className="text-2xl font-mono text-slate-800 dark:text-white">
                      {wearResult !== null
                        ? wearResult > 0
                          ? "x" + wearResult.toFixed(3)
                          : t("Error!")
                        : "---"}
                    </div>
                  </div>
                    ...
                    <>
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="text-xl font-mono text-indigo-300">
                          {Math.floor(95 / ((wearResult || 1) * 3.2 * 1.45))}{" "}
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="text-xl font-mono text-indigo-300">
                          {Math.floor(95 / ((wearResult || 1) * 2.1 * 1.45))}{" "}
                          <span className="text-sm">Laps</span>
                        </div>
                      </div>
                    </>
                ...
                <div className="mt-auto pt-8 flex-1 flex items-end">
                  <div className="w-full">
                    ...
                    <p className="text-center text-xs text-slate-400 mt-3 font-medium">
                      {t("Values will be synced to your account.")}
                    </p>
                  </div>
                </div>
              )}
