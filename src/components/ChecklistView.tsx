...
export default function ChecklistView({
  ...
  const [items, setItems] = useState([
    {
      text: "Check weather forecast for Q and R",
      done: false,
      tab: "track_setup",
    },
    {
      text: "Adjust car setup for current temperature",
      done: false,
      tab: "driver_profile",
    },
    ...
    {
      text: "Confirm pit strategy (stints/laps)",
      done: false,
      tab: "strategy",
    },
    {
      text: "Check and configure secondary strategy for rain",
      done: false,
      tab: "strategy",
    },
    {
      text: "Verify part wear and replace if necessary",
      done: false,
      tab: "driver_profile",
    },
  ...
  const save = async () => {
    ...
    try {
      setIsSaving(true);
      await setDoc(
        doc(db, "checklists", currentUser.uid),
        {
          items,
        },
        { merge: true },
  ...
  const handleChat = async () => {
    ...
    setTimeout(() => {
      let reply = t(
        "I'm not sure about that. Try adjusting your setup based on test laps.",
      );
      const m = msg.toLowerCase();

      if (m.includes("rain") || m.includes("yağmur")) {
        reply = t(
          "If it rains, make sure you have a secondary strategy configured to pit for Rain tyres when the water level exceeds your threshold (usually around 1-3mm).",
        );
      } else if (m.includes("fuel") || m.includes("yakıt")) {
        reply = t(
          "Check the Strategy tab. Make sure your total fuel covers the target laps + 1 extra lap for safety, but don't overfuel as it costs lap time (approx 0.03s per liter).",
        );
      } else if (m.includes("tyre") || m.includes("lastik")) {
        reply = t(
          "Tyre wear depends heavily on Track Temperature. Use harder compounds for hot tracks, and softer ones for cold tracks. Make sure the total estimated wear doesn't exceed 100%.",
        );
      } else if (m.includes("setup") || m.includes("ayar")) {
        reply = t(
          "Your car setup should be calibrated using Test Laps. Adjust Wings, Engine, and Brakes based on the track characteristics (Power vs Handling).",
        );
      } else if (
        m.includes("risk") ||
        m.includes("aggression") ||
        m.includes("agresif")
      ) {
        reply = t(
          "Higher driver risk yields faster laps but drastically increases part wear and tyre wear, plus higher chance of mistakes. Use it sparingly!",
        );
      }
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 p-5 rounded-2xl shadow-sm">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
        <h2 className="text-lg font-semibold flex items-center gap-2 text-indigo-900 dark:text-indigo-400">
          ...
          {t("Pre-Race Checklist")}{" "}
        </h2>
        <div className="flex items-center gap-2 sm:self-auto self-end">
          <button
            onClick={reset}
            className="px-2 py-1 flex items-center justify-center text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 font-bold transition rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"
          >
      ...
      <div className="mt-8 p-5 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 rounded-2xl w-full flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-indigo-900 dark:text-indigo-300 leading-tight">
              {t("Race Assistant")}
            </h3>
            <p className="text-xs text-indigo-700/80 dark:text-indigo-400/80">
              {t("Need help preparing for the race?")}
            </p>
          </div>
        </div>
        ...
        {chatLog.length === 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-indigo-100/50 dark:border-indigo-800/50 text-sm font-medium text-slate-700 dark:text-slate-300">
            {items.every((i) => i.done) ? (
              ...
              <p>
                {t("Welcome to race day! Start by configuring your")}{" "}
                <span
                  className="text-indigo-600 dark:text-indigo-400 cursor-pointer hover:underline"
                  onClick={() => goToTab("track_setup")}
                >
                  {t("Track Setup")}
                </span>{" "}
                {t("to pull in the correct Track data, then check your")}{" "}
                <span
                  className="text-indigo-600 dark:text-indigo-400 cursor-pointer hover:underline"
                  onClick={() => goToTab("calibration")}
                >
                  {t("Calibration")}
                </span>{" "}
                {t("limits.")}
        ...
        <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-2">
          {chatLog.map((log, idx) => (
            <div
              key={idx}
              className={`p-3 rounded-xl max-w-[90%] text-sm ${log.sender === "user" ? "bg-indigo-600 text-white self-end rounded-br-none" : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 self-start rounded-bl-none"}`}
            >
              {log.text}
            </div>
          ))}
