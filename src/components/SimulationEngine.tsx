...
const InfoTooltip = ({ text }: { text: string }) => (
  <div
    title={text}
    className="ml-1.5 inline-flex items-center justify-center align-middle cursor-help"
  >
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (pathRef.current && containerRef.current && simRef.current) {
        ...
        const playerPoint = pathRef.current.getPointAtLength(
          pathLength * playerProg,
        );
...
export default function SimulationEngine({
  ...
  const activeTrack = useMemo(
    () => TRACK_DATABASE.find((t) => t.id === selectedTrackId) || initialTrack,
  ...
  const [pitRefuelStrategyStr, setPitRefuelStrategyStr] =
    useState<string>("140");
  ...
  const estimatedCarWear = useMemo(
    () => calculateRaceCarWear(calcParams),
    [calcParams],
  );
  ...
  const resetSim = () => {
    const bots = customBots.map((b, i) => ({
      ...
      color:
        b.color ||
        ["#ef4444", "#f59e0b", "#10b981", "#3b82f6", "#8b5cf6"][i % 5],
      status: "Running",
    }));
  ...
  const calculateLapTargets = (s: typeof sim.current) => {
    ...
    s.lapWearIncrease = calculateWearPerLap(
      compound,
      calcParams,
      weather.rainProps.q1,
    );
      ...
      compound === "XS"
        ...
        : compound === "S"
          ...
          : compound === "M"
            ? 1.2
            : 2.0;
    const tyreDegSpeedPenalty =
      Math.pow(Math.max(0, s.tyreWear) / 100, 2) * 5.0;
    ...
    if (s.isRunning && s.status === "Running") {
      ...
      Object.keys(s.carWear).forEach((part) => {
        ...
        if (s.carWear[key] > 90 && Math.random() < 0.0001) {
          ...
          s.logs.unshift(
            `[Lap ${s.lap}] ⚠️ ${(key as string).toUpperCase()} malfunction! Losing pace.`,
          );
        }
      });
      ...
      s.bots.forEach((b: any) => {
        if (b.status === "Running") {
          ...
          const distanceDiff =
            (playerTotal - botTotal) * activeTrack.distance * 1000;
      ...
      if (s.lapProgress >= 1) {
        ...
        s.energy -= Math.max(
          0.1,
          1.0 + tempStrain + riskClear * 0.01 - staminaBonus,
        );
        ...
        if (dnfPart) {
          ...
          s.logs.unshift(
            `[Lap ${s.lap}] 💥 DNF due to catastrophic ${dnfPart} failure.`,
          );
          s.isRunning = false;
          ...
          s.logs.unshift(
            `[Lap ${s.lap}] 💥 DNF: Tyre Puncture! Strategy pushed too far.`,
          );
          s.isRunning = false;
          ...
          s.logs.unshift(
            `🏁 Checkered Flag! Finished in ${h > 0 ? h + "h " : ""}${m}m ${(s.totalTime % 60).toFixed(3)}s. (${s.pits} pits)`,
          );
          s.isRunning = false;
          ...
          if (s.fuel <= 0) {
            ...
            s.logs.unshift(
              `[Lap ${s.lap}] ⛽ Fuel empty! Crawling into the pitlane.`,
            );
          }
          s.status = "Pitting";
          s.message = `Pit Stop!`;
          s.logs.unshift(
            `[Lap ${s.lap}] 🔧 Entered Pitlane. Tyre condition was ${(100 - s.tyreWear).toFixed(1)}%.`,
          );
          s.pitTimer =
            activeTrack.pitTimeBase +
            (pitRefuelStrategy - Math.max(0, s.fuel)) /
      ...
      if (s.pitTimer <= 0) {
        ...
        s.logs.unshift(
          `[Lap ${s.lap}] 🟢 Exited Pitlane. New ${selectedTyre} tyres, refuelled to ${s.fuel}L.`,
        );
        calculateLapTargets(s);
        s.message = `Running Lap ${s.lap + 1}...`;
      }
  ...
  const toggleSim = () => {
    if (
      !isRunning &&
      sim.current.status !== "Finished" &&
      sim.current.status !== "DNF"
    ) {
      setIsRunning(true);
      ...
      if (sim.current.lap === 0) {
        ...
        sim.current.logs.unshift(
          `🟢 Race Started at ${activeTrack.name}! Temp: ${weather.tempBase}°C. Strategy: ${selectedTyre} tyres, ${sim.current.fuel}L fuel.`,
        );
  ...
  const displayTyreWear = Math.min(
    100,
    uiState.tyreWear + sim.current.lapWearIncrease * uiState.lapProgress,
  );
  const displayFuel = Math.max(
    0,
    uiState.fuel - sim.current.lapFuelUsage * uiState.lapProgress,
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition"
          >
            <ChevronLeft className="w-5 h-5 text-slate-600 dark:text-slate-300" />
          </button>
          <div>
            <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <Activity className="w-6 h-6 text-indigo-500" /> Lab Simulator{" "}
              <span className="px-2 py-0.5 text-[10px] bg-indigo-100 text-indigo-600 font-bold uppercase rounded-md tracking-wider">
                Live
              </span>
            </h2>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-1">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Run laps continuously. Adjust parameters mid-race to see effects
                instantly.
              </p>
              <span className="text-[10px] bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-800 px-2 py-0.5 rounded-full font-bold">
                ⚠️ Early Development Phase: Free to use for now, but may be
                restricted later.
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-900 p-5 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-sm">
            <h3 className="font-bold text-sm text-slate-800 dark:text-slate-200 uppercase tracking-widest mb-4">
              Parameters
            </h3>
            ...
            <div className="space-y-5">
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    disabled={
                      isRunning ||
                      uiState.lap > 0 ||
                      selectedTrackId === initialTrack.id
                    }
                ...
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm font-bold text-slate-800 dark:text-slate-200 cursor-pointer disabled:opacity-50"
                >
              ...
              <div>
                ...
                <select
                  value={selectedTyre}
                  onChange={(e) => setSelectedTyre(e.target.value as any)}
                  disabled={isRunning || uiState.lap > 0}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm font-bold text-slate-800 dark:text-slate-200 cursor-pointer disabled:opacity-50"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={startFuelStr}
                    onChange={(e) => {
                      setStartFuelStr(e.target.value);
                      if (uiState.lap === 0 && !isNaN(Number(e.target.value)))
                        setUiState((u) => ({
                          ...u,
                          fuel: Number(e.target.value),
                        }));
                    }}
                    className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm font-bold disabled:opacity-50"
                    disabled={isRunning || uiState.lap > 0}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    value={pitRefuelStrategyStr}
                    onChange={(e) => setPitRefuelStrategyStr(e.target.value)}
                    className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm font-bold"
                    disabled={isRunning}
                  />
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 mb-1.5 flex items-center justify-between uppercase">
                  <span className="flex items-center">
                    Condition Limit{" "}
                    <InfoTooltip text="Forces a pit stop if tyre condition drops below this percentage." />
                  </span>
                  <span className="font-mono text-indigo-500">
                    {pitConditionLimit === 0 ? "OFF" : `${pitConditionLimit}%`}
                  </span>
                </label>
                <input
                  type="range"
                  ...
                  max="100"
                  step="5"
                  value={pitConditionLimit}
                  onChange={(e) =>
                    setPitConditionLimit(parseInt(e.target.value))
                  }
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1 gap-0.5 overflow-x-auto">
                  {[1, 5, 10, 20, 50, 100, 200, 500].map((s) => (
                    <button
                      ...
                      className={`px-2 py-1 flex-shrink-0 text-[10px] font-bold rounded-md ${speedMultiplier === s ? "bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400" : "text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600/50"}`}
                    >
                      {s}x
                    </button>
                  ))}
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-2 mb-3">
                  {customBots.map((bot, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/80 p-2 rounded border border-slate-100 dark:border-slate-700"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-slate-500">
                          {bot.basePace.toFixed(2)}s pace
                        </span>
                        <button
                          onClick={() =>
                            setCustomBots(
                              customBots.filter((_, idx) => idx !== i),
                            )
                          }
                          disabled={isRunning || uiState.lap > 0}
                          className="text-red-400 hover:text-red-600 font-bold px-1 disabled:opacity-30"
                        >
                          ×
                        </button>
                      </div>
                  ...
                  {customBots.length === 0 && (
                    <p className="text-xs text-slate-400 italic text-center py-2">
                      No opponents. You are racing alone.
                    </p>
                  )}
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    disabled={isRunning || uiState.lap > 0}
                    className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 text-xs font-bold"
                  />
                  <input
                    type="text"
                    placeholder="Pace (s)"
                    value={newBotPaceStr}
                    onChange={(e) => setNewBotPaceStr(e.target.value)}
                    disabled={isRunning || uiState.lap > 0}
                    className="text-slate-800 dark:text-slate-100 w-20 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 text-xs font-mono font-bold text-center"
                  />
                  <button
                    onClick={() => {
              ...
              <div className="flex gap-2">
                <button
                  onClick={toggleSim}
                  className={`flex-1 py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-white transition-all transform active:scale-[0.98] ${isRunning ? "bg-orange-500 hover:bg-orange-600 shadow-orange-500/20" : uiState.status === "Finished" || uiState.status === "DNF" ? "bg-slate-400 cursor-not-allowed" : "bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/20"} shadow-md`}
                  disabled={
                    uiState.status === "Finished" || uiState.status === "DNF"
                  }
                >
                  {isRunning ? (
                    <>
                    ...
                    <>
                      ...
                      {uiState.lap > 0 ? "RESUME" : "START"}
                    </>
                  )}
                </button>
                <button
                  onClick={resetSim}
                  className="px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition"
          ...
          <div className="bg-white dark:bg-slate-900 p-5 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-sm">
            ...
            <div className="mt-6 flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <div className="text-xs font-bold uppercase tracking-widest mb-1 text-slate-500">
                  Live Status
                </div>
                <div
                  className={`text-xl font-bold font-mono tracking-tight ${uiState.status === "DNF" ? "text-red-500" : uiState.status === "Finished" ? "text-green-500" : "text-slate-800 dark:text-slate-100"}`}
                >
                  {uiState.message}
                </div>
                ...
                <div className="mt-5 grid grid-cols-2 lg:grid-cols-6 gap-4">
                  ...
                  <div>
                    ...
                    <div className="font-mono text-lg font-bold text-slate-800 dark:text-slate-100">
                      {Math.max(0, 100 - displayTyreWear).toFixed(1)}%
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase text-slate-500 mb-1">
                  ...
                  <div>
                    ...
                    <div
                      className={`font-mono text-lg font-bold flex items-baseline gap-1 ${uiState.tyreTemp && uiState.tyreTemp > 105 ? "text-orange-500" : "text-slate-800 dark:text-slate-100"}`}
                    >
                      {uiState.tyreTemp ? uiState.tyreTemp.toFixed(1) : "90.0"}
              ...
              <div className="w-[300px] bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden hidden xl:flex">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2 h-40">
                  {uiState.logs?.length === 0 && (
                    <div className="text-xs text-slate-400 dark:text-slate-600 font-mono italic">
                      Waiting for events...
                    </div>
                  )}
                  {uiState.logs?.map((msg, idx) => (
                    <div
                      key={idx}
                      className="text-[11px] font-mono text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-800 pb-1.5 last:border-0"
                    >
                      {msg}
                    </div>
                  ))}
              ...
              <div className="min-w-[200px] bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700/60 hidden lg:block">
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs font-mono">
                  {Object.entries(estimatedCarWear)
                    .slice(0, 8)
                    .map(([part, wear]) => (
                      <div
                        key={part}
                      ...
                      >
                        ...
                        <span
                          className={`font-bold ${uiState.carWear[part] > 75 ? "text-red-500" : uiState.carWear[part] > 50 ? "text-orange-500" : "text-slate-700 dark:text-slate-200"}`}
                        >
              ...
              <div className="w-full max-w-4xl px-8 relative z-10">
                ...
                <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded-full border border-slate-300 dark:border-slate-700 relative overflow-visible">
                  ...
                  <div
                    ...
                    style={{
                      left: `${Math.min(100, Math.max(0, uiState.lapProgress * 100))}%`,
                    }}
                  >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-0.5 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                  {[5, 10, 20, 30, 50, 100, "Max"].map((w) => (
                    <button
                      key={w}
                      onClick={() => setLapWindow(w as any)}
                      className={`px-2 py-0.5 text-[10px] font-bold rounded-md transition ${lapWindow === w ? "bg-white dark:bg-slate-700 shadow-sm text-slate-800 dark:text-slate-200" : "text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600"}`}
                    >
                      {w}
                    </button>
                  ))}
              ...
              >
                <AreaChart
                  data={
                    lapWindow === "Max"
                      ? lapData
                      : lapData.slice(-(lapWindow as number))
                  }
                  syncId="simCharts"
                >
                  <defs>
            ...
            <div className="bg-white dark:bg-slate-900 p-5 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-sm h-64">
              ...
              <ResponsiveContainer width="100%" height="100%" className="pb-4">
                ...
                >
                  ...
                  <Area
                    type="monotone"
                    dataKey={(d: any) => Math.max(0, 100 - d.tyreWear)}
                    stroke="#22c55e"
                    fillOpacity={1}
                    fill="url(#colorWear)"
                    strokeWidth={2}
