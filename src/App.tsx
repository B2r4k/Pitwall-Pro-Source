...
const PrepDot = ({
  ...
  if (isBlindZone) {
    return (
      <div
        className="absolute -top-1.5 -right-1.5 flex h-3 w-3 z-20 pointer-events-none"
        title="Race Time (Blind Zone)"
      >
  ...
  return (
    <div
      className="absolute -top-1.5 -right-1.5 flex h-3 w-3 z-20 pointer-events-none"
      title={isReady ? "Prepared" : "Needs Preparation"}
    >
      <span
        className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-60 ${isReady ? "bg-emerald-400" : "bg-red-400"}`}
      ></span>
      <span
        className={`relative inline-flex rounded-full h-3 w-3 border border-white dark:border-slate-800 shadow-sm ${isReady ? "bg-emerald-500" : "bg-red-500"}`}
      ></span>
    </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    const checkRaceTime = () => {
      ...
      if ((utcDay === 2 || utcDay === 5) && utcHour >= 18 && utcHour < 20) {
          ...
          return [
            {
              id: "live-race-active",
              type: "in-game",
              title: "Live Race Active!",
              message:
                "We are currently within race hours. Jump into the race when you are ready to apply your strategies!",
        ...
        setSystemLogs((prev) =>
          prev.filter((l) => l.id !== "live-race-active"),
        );
      }
    };
    checkRaceTime();
  ...
  const addSystemLog = (
    title: string,
    message: string,
    type: "in-game" | "patch-note" = "in-game",
  ) => {
    setSystemLogs((prev) => [
      {
        id: "log-" + Date.now(),
  ...
  const [telemetryState, setTelemetryState] = useState({
    ...
    includeParts: true,
    form: {
      trackId: "australia", laps: 60, tempBase: 20, tempMax: 25, rainProb: 0,
      driverFocus: 250, driverStamina: 250, driverExperience: 0, driverWeight: 80,
      aggression: 20, clearRisk: 0, defendRisk: 0,
      power: 20, handling: 20, acceleration: 20, baseFuelPerLap: 1.8, tyreSupplier: "Pipirelli",
      compound: "Soft", outFuelUsed: 110, outTyreWear: 80, outPartsWearAvg: 12
    }
  });
  ...
  const markAllNotificationsAsRead = () => {
    ...
    if (all.length > 0) {
      setLocalSettings((s) => {
        const dynamicIds = ["alert-upcoming-race", "alert-high-risk", "alert-rain", "alert-extreme-risk-rain", "alert-high-wear", "alert-low-stamina", "alert-cold-track", "alert-hot-track", "alert-high-fuel"];
        const currentIds = all.map(n => n.id);
        const nextDismissed = Array.from(new Set([...(s.dismissedAlerts || []), ...currentIds]));
        const cleanedDismissed = nextDismissed.filter(id => currentIds.includes(id) || dynamicIds.includes(id));
        return {
          ...s,
  ...
  const isNotificationNew = (note: any) => {
    return !(settings.dismissedAlerts || []).includes(note.id);
  };

  const hasUnreadNotifications = useMemo(() => {
    const all = [...systemLogs, ...APP_NOTIFICATIONS];
    return all.some(note => isNotificationNew(note));
  }, [systemLogs, settings.dismissedAlerts]);
  ...
  const [customTrack, setCustomTrack] = useState<Track>(
    TRACK_DATABASE.find((t) => t.id === "custom")!,
  );
  ...
  useEffect(() => {
    ...
    if (
      weather.tempBase !== prevWeatherRef.current.tempBase ||
      weather.tempMax !== prevWeatherRef.current.tempMax ||
      weather.rainProps !== prevWeatherRef.current.rainProps
    )
      updates.weather = now;
    ...
    if (p.driverExperience !== prevP.driverExperience)
      updates.driverExperience = now;
    if (p.riskAggression !== prevP.riskAggression) updates.riskAggression = now;
    if (p.energy !== prevP.energy) updates.driverEnergy = now;
    if (p.baseWearMultiplier !== prevP.baseWearMultiplier)
      updates.driverWearMultiplier = now;
    ...
    if (
      p.pha?.power !== prevP.pha?.power ||
      p.pha?.handling !== prevP.pha?.handling ||
      p.pha?.acceleration !== prevP.pha?.acceleration
    )
      updates.carPHA = now;
  ...
  const resetToDefaults = () => {
    ...
    setWeather({
      tempBase: 20, tempMax: 25,
      q1: "Dry", q2: "Dry", r1: "Dry", r2: "Dry", r3: "Dry", r4: "Dry",
      rainProps: { q1: 0, q2_r1: 0, r2: 0, r3: 0, r4: 0 },
    });
    setCustomTrack(TRACK_DATABASE.find((t) => t.id === "custom")!);
    setLocalSettings({
      theme: "system", language: "en", autoTranslate: false, gproToken: "", notifications: false,
      smartSync: true, lastViewedTimestamp: 0, onboardingSeen: true, autoApiSync: false, lastApiSyncTime: 0,
      dismissedAlerts: [],
    ...
    setActiveProfileIdx(0);
    setConstants({
      fuelPerLitreKg: 0.75, pitStopBaseTime: 22.0, timeLossPerLapWear: 0.05,
      fuelTimePenaltyPerKg: 0.035, riskMultiplier: 1.0, rainTimeLoss: 1.2,
    });
    setSelectedTrackId("melbourne");
    setTelemetryState({
      includeFuel: true, includeTyres: true, includeParts: true,
      form: { trackId: "australia", laps: 60, tempBase: 20, tempMax: 25, rainProb: 0, driverFocus: 250, driverStamina: 250, driverExperience: 0, driverWeight: 80, aggression: 20, clearRisk: 0, defendRisk: 0, power: 20, handling: 20, acceleration: 20, baseFuelPerLap: 1.8, tyreSupplier: "Pipirelli", compound: "Soft", outFuelUsed: 110, outTyreWear: 80, outPartsWearAvg: 12 }
    });
  };
  ...
  const loadFromCloudData = async (activeUid: string | null, shouldSetSyncing = true) => {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.telemetryState) setTelemetryState(data.telemetryState);
        if (shouldSetSyncing) {
    ...
    if (settings.smartSync) {
      const timeout = setTimeout(async () => {
        try {
          const payload = {
            savedProfiles: (() => {
              const p = [...savedProfiles];
              p[activeProfileIdx] = player;
              return p.length > 0 ? p : [player];
            })(),
  ...
  const resetPlayerSettings = () => {
    if (
      confirm(
        t("Are you sure you want to reset driver and car settings to default?"),
      )
    ) {
  ...
  const exportProfileToFile = () => {
    try {
      const dataStr =
        ...
        encodeURIComponent(JSON.stringify(player, null, 2));
      const downloadAnchorNode = document.createElement("a");
      downloadAnchorNode.setAttribute("href", dataStr);
      downloadAnchorNode.setAttribute(
        "download",
        `gpro-profile-${new Date().toISOString().split("T")[0]}.json`,
      );
  ...
  const applyApiData = () => {
    ...
    setPlayer((p) => {
      const newP = { ...p };
      let changed = false;
      const opt = settings.apiSyncOptions || {
        driver: true,
        car: true,
        race: true,
      };
      ...
      if (opt.driver && scrapedData) {
        if (
          scrapedData.energy !== undefined &&
          newP.energy !== scrapedData.energy
        ) {
          newP.energy = scrapedData.energy;
          changed = true;
        }
        if (
          scrapedData.weight !== undefined &&
          newP.weight !== scrapedData.weight
        ) {
          newP.weight = scrapedData.weight;
          changed = true;
        }
        if (
          scrapedData.focus !== undefined &&
          newP.focus !== scrapedData.focus
        ) {
          newP.focus = scrapedData.focus;
          changed = true;
        }
        if (
          scrapedData.stamina !== undefined &&
          newP.stamina !== scrapedData.stamina
        ) {
          newP.stamina = scrapedData.stamina;
          changed = true;
        }
        if (
          scrapedData.experience !== undefined &&
          newP.experience !== scrapedData.experience
        ) {
          newP.experience = scrapedData.experience;
          changed = true;
        }
  ...
  const fetchGproApi = async (isAuto = false) => {
    if (!settings.gproToken) {
      if (!isAuto)
        alert(
          t("Lütfen Settings kısmından GPRO API Token (JWT) bilginizi girin."),
        );
      return;
    }
    if (isSyncing) return;
    ...
    try {
      ...
      if (endpoints.length === 0) {
        setIsSyncing(false);
        if (!isAuto) {
          setSyncStatus("No modules selected to fetch.");
          setTimeout(() => setSyncStatus(""), 3000);
        }
        return;
      }
      ...
      try {
        responses = await Promise.all(
          endpoints.map((ep) =>
            fetch("/api/gpro?endpoint=" + encodeURIComponent(ep.url), {
              method: "GET",
              headers,
        ...
        if (fetchErr.name === 'AbortError') {
          throw new Error("API isteği zaman aşımına uğradı (10s). Lütfen tekrar deneyin.");
        }
        throw fetchErr;
      } finally {
      ...
      if (!scrapedData.energy && !scrapedData.carPower) {
        throw new Error(
          "GPRO API Fetch failed or empty data. Token may be invalid.",
        );
      }
      ...
      if (
        currentUser &&
        currentUser.uid &&
        scrapedData.concentration !== undefined
      ) {
        try {
          const { serverTimestamp } = await import("firebase/firestore");
          await setDoc(
            doc(db, "driverStats", currentUser.uid),
            {
              userId: currentUser.uid,
          ...
          console.error(
            "Failed to persist raw Driver Stats to Cloud:",
            firebaseErr,
          );
        }
      }
      ...
      const trackSrc =
        apiData.trackProfile || apiData.practice || apiData.setup || {};
      if (
        checkFilter("track") &&
        (trackSrc.trackId || trackSrc.trackName || trackSrc.name)
      ) {
        previewObj.track = {
      ...
      if (checkFilter("weather") && weatherSrc) {
        previewObj.weather = {
          ...
          rainProps: {
            ...
            q2_r1:
              weatherSrc.raceQ1RainPHigh !== undefined
                ? Math.round(
                    (weatherSrc.raceQ1RainPLow + weatherSrc.raceQ1RainPHigh) /
                      2,
                  )
                : undefined,
            r2:
              weatherSrc.raceQ2RainPHigh !== undefined
                ? Math.round(
                    (weatherSrc.raceQ2RainPLow + weatherSrc.raceQ2RainPHigh) /
                      2,
                  )
                : undefined,
            r3:
              weatherSrc.raceQ3RainPHigh !== undefined
                ? Math.round(
                    (weatherSrc.raceQ3RainPLow + weatherSrc.raceQ3RainPHigh) /
                      2,
                  )
                : undefined,
            r4:
              weatherSrc.raceQ4RainPHigh !== undefined
                ? Math.round(
                    (weatherSrc.raceQ4RainPLow + weatherSrc.raceQ4RainPHigh) /
                      2,
                  )
                : undefined,
          },
  ...
  const applyApiDataFn = (dataPreview, isAuto = false) => {
    setPlayer((p) => {
      ...
      if (dataPreview.driver) {
        ...
        if (d.energy !== undefined && newP.energy !== d.energy) {
          newP.energy = d.energy;
          changed = true;
        }
        if (d.focus !== undefined && newP.driverFocus !== d.focus) {
          newP.driverFocus = d.focus;
          changed = true;
        }
        if (d.stamina !== undefined && newP.driverStamina !== d.stamina) {
          newP.driverStamina = d.stamina;
          changed = true;
        }
        if (
          d.experience !== undefined &&
          newP.driverExperience !== d.experience
        ) {
          newP.driverExperience = d.experience;
          changed = true;
        }
        if (
          d.aggression !== undefined &&
          newP.riskAggression !== d.aggression
        ) {
          newP.riskAggression = d.aggression;
          changed = true;
        }
      ...
      if (dataPreview.car) {
        if (
          dataPreview.car.power !== undefined &&
          newP.carPower !== dataPreview.car.power
        ) {
          newP.carPower = dataPreview.car.power;
          changed = true;
        }
        if (
          dataPreview.car.handling !== undefined &&
          newP.carHandling !== dataPreview.car.handling
        ) {
          newP.carHandling = dataPreview.car.handling;
          changed = true;
        }
        if (
          dataPreview.car.acceleration !== undefined &&
          newP.carAcceleration !== dataPreview.car.acceleration
        ) {
          newP.carAcceleration = dataPreview.car.acceleration;
          changed = true;
        }
      ...
      if (dataPreview.risks) {
        ...
        if (r.overtake !== undefined && newP.risks.overtake !== r.overtake) {
          newP.risks.overtake = r.overtake;
          changed = true;
        }
        if (r.defend !== undefined && newP.risks.defend !== r.defend) {
          newP.risks.defend = r.defend;
          changed = true;
        }
        if (r.clearDry !== undefined && newP.risks.clearDry !== r.clearDry) {
          newP.risks.clearDry = r.clearDry;
          changed = true;
        }
        if (r.clearWet !== undefined && newP.risks.clearWet !== r.clearWet) {
          newP.risks.clearWet = r.clearWet;
          changed = true;
        }
        if (
          r.malfunction !== undefined &&
          newP.risks.malfunction !== r.malfunction
        ) {
          newP.risks.malfunction = r.malfunction;
          changed = true;
        }
    ...
    if (dataPreview.track && dataPreview.track.name) {
      ...
      const match = TRACK_DATABASE.find(
        (t) => t.name.toLowerCase() === dataPreview.track.name.toLowerCase(),
      );
      if (match) {
    ...
    if (dataPreview.weather) {
      setWeather((w) => {
        ...
        if (dataPreview.weather.raceTempLow !== undefined) {
          const avg =
            (dataPreview.weather.raceTempLow +
              dataPreview.weather.raceTempHigh) /
            2;
          if (!isNaN(avg)) {
        ...
        if (dataPreview.weather.rainProps) {
          if (!nw.rainProps)
            nw.rainProps = { q1: 0, q2_r1: 0, r2: 0, r3: 0, r4: 0 };
          nw.rainProps = { ...nw.rainProps };

          if (dataPreview.weather.rainProps.q1 !== undefined) {
            nw.rainProps.q1 = dataPreview.weather.rainProps.q1;
            changed = true;
          }
          if (dataPreview.weather.rainProps.q2_r1 !== undefined) {
            nw.rainProps.q2_r1 = dataPreview.weather.rainProps.q2_r1;
            changed = true;
          }
          if (dataPreview.weather.rainProps.r2 !== undefined) {
            nw.rainProps.r2 = dataPreview.weather.rainProps.r2;
            changed = true;
          }
          if (dataPreview.weather.rainProps.r3 !== undefined) {
            nw.rainProps.r3 = dataPreview.weather.rainProps.r3;
            changed = true;
          }
          if (dataPreview.weather.rainProps.r4 !== undefined) {
            nw.rainProps.r4 = dataPreview.weather.rainProps.r4;
            changed = true;
          }
        }
        return changed ? nw : w;
  ...
  const guessCurrentTrack = async (isAuto = false) => {
    try {
      ...
      if (isAuto) {
        ...
        const pastRaces = F1_CALENDAR_S113.filter(
          (r) => new Date(r.date).getTime() < now,
        );
        const nextRaces = F1_CALENDAR_S113.filter(
          (r) => new Date(r.date).getTime() >= now,
        );
        if (pastRaces.length > 0 && nextRaces.length > 0) {
          const lastRaceTime = new Date(
            pastRaces[pastRaces.length - 1].date,
          ).getTime();
          const nextRaceTime = new Date(nextRaces[0].date).getTime();
          if (!(
            now > lastRaceTime + 2 * 3600 * 1000 &&
            now < nextRaceTime - 1.5 * 3600 * 1000
          )) {
      ...
      let nextRace = calendarToUse.find(
        (r) => new Date(r.date).getTime() > now,
      );
      ...
      if (nextRace) {
        ...
        if (
          !isAuto ||
          (isAuto && !customTrack && selectedTrackId === "melbourne")
        ) {
          ...
          if (!isAuto) {
            setSyncStatus(t("Calendar Sync:") + " " + nextRace.name);
      ...
      if (!isAuto) {
        {
          setSyncStatus(t("Calendar disconnected or error occurred."));
          setTimeout(() => setSyncStatus(""), 4000);
        }
      }
    } finally {
  ...
  useEffect(() => {
    const performAutoSync = async () => {
      ...
      if (lastRace && nextRace) {
        const twoHoursAfterLastRace =
          new Date(lastRace.date).getTime() + 2 * 60 * 60 * 1000;
        const oneAndHalfHoursBeforeNextRace =
          new Date(nextRace.date).getTime() - 1.5 * 60 * 60 * 1000;

        if (
          now >= twoHoursAfterLastRace &&
          now <= oneAndHalfHoursBeforeNextRace
        ) {
          if (
            !settings.lastApiSyncTime ||
            settings.lastApiSyncTime < twoHoursAfterLastRace
          ) {
  ...
  const extractNumberNearKeyword = (
    text: string,
    keywords: string[],
  ): number | null => {
  ...
  const processImage = async (file: File) => {
    ...
    setOcrStatus(
      "Scanning image, please wait. Clearly cropped images give better results.",
    );
    setOcrPreview(null);
    try {
      ...
      if (tempMatch) {
        if (parseInt(tempMatch[1], 10) < 50)
          parsedData.tempBase = parseInt(tempMatch[1], 10);
        if (parseInt(tempMatch[2], 10) < 50)
          parsedData.tempMax = parseInt(tempMatch[2], 10);
      } else {
        // Fallback to keyword search
        const tempT1 = extractNumberNearKeyword(text, [
          "min temp",
          "min sıcaklık",
          "temperature",
          "sıcaklık",
        ...
        const tempT2 = extractNumberNearKeyword(text, [
          "max temp",
          "max sıcaklık",
          "highest",
        ]);
      ...
      const percs = Array.from(text.matchAll(percRegex)).map((m) =>
        parseInt(m[1], 10),
      );
      if (percs.length >= 2) {
      ...
      const dWeight = extractNumberNearKeyword(text, [
        "weight",
        "ağırlık",
        "kilo",
      ]);
      if (dWeight && dWeight > 40 && dWeight < 150)
        parsedData.driverWeight = dWeight;

      const focus = extractNumberNearKeyword(text, [
        "concentration",
        "konsantrasyon",
      ]);
      if (focus) parsedData.driverFocus = focus;

      const stamina = extractNumberNearKeyword(text, [
        "stamina",
        "dayanıklılık",
      ]);
      if (stamina) parsedData.driverStamina = stamina;

      const experience = extractNumberNearKeyword(text, [
        "experience",
        "deneyim",
        "tecrübe",
      ]);
      ...
      const risk = extractNumberNearKeyword(text, [
        "risk",
        "agresiflik",
        "agressiandness",
        "aggression",
      ...
      if (Object.keys(parsedData).length > 0) {
        ...
        setOcrStatus(
          t("No clear data found, try cropping the area more precisely."),
        );
      ...
      addSystemLog(
        "Akıllı Görüntü Okuma Hatası",
        "Ekran görüntüsü okunamadı: " + (e as Error).message,
        "patch-note",
      );
  ...
  const applyOcrData = () => {
    ...
    if (ocrPreview.tempBase)
      setWeather((w) => ({ ...w, tempBase: ocrPreview.tempBase! }));
    if (ocrPreview.tempMax)
      setWeather((w) => ({ ...w, tempMax: ocrPreview.tempMax! }));
    if (ocrPreview.rainProps)
      setWeather((w) => ({ ...w, rainProps: ocrPreview.rainProps! }));

    // Only update if there are determined fields
    const hasPlayerUpdates =
      ocrPreview.driverFocus !== undefined ||
      ocrPreview.driverStamina !== undefined ||
      ocrPreview.driverExperience !== undefined ||
      ocrPreview.riskAggression !== undefined;
    ...
    if (hasPlayerUpdates) {
      setPlayer((p) => ({
        ...p,
        ...(ocrPreview.driverFocus !== undefined
          ? { driverFocus: ocrPreview.driverFocus }
          : {}),
        ...(ocrPreview.driverStamina !== undefined
          ? { driverStamina: ocrPreview.driverStamina }
          : {}),
        ...(ocrPreview.driverExperience !== undefined
          ? { driverExperience: ocrPreview.driverExperience }
          : {}),
        ...(ocrPreview.riskAggression !== undefined
          ? { riskAggression: ocrPreview.riskAggression }
          : {}),
      }));
    }
  ...
  const handleDownloadProtocol = () => {
    ...
    lines.push(
      `Track: ${activeTrack.name} (${activeTrack.laps} Laps | ${activeTrack.distance} km)`,
    );
    lines.push(`Driver: ${player.name}`);
    lines.push(
      `Temperature: ${weather.tempMax}°C | Rain Prob: ${weather.rainProps ? Math.max(weather.rainProps.q1, weather.rainProps.q2_r1, weather.rainProps.r2, weather.rainProps.r3, weather.rainProps.r4) : 0}%`,
    );
    lines.push(`Total Stops: ${baseSelectedStrategy.stops}`);
    lines.push(
      `Estimated Relative Race Time: +${baseSelectedStrategy.totalRaceTime.toFixed(1)} sec`,
    );
    lines.push(``);
    baseSelectedStrategy.stints.forEach((stint, idx) => {
      lines.push(`Stint ${idx + 1}:`);
      lines.push(
        `  Tyres: ${stint.tyres} (${COMPOUND_FULL_NAMES[stint.tyres]})`,
      );
  ...
  const handleCopyProtocol = async () => {
    ...
    lines.push(`🚗 *Driver:* ${player.name}`);
    lines.push(
      `🌡️ *Temp:* ${weather.tempMax}°C | *Rain Prob:* ${weather.rainProps ? Math.max(weather.rainProps.q1, weather.rainProps.q2_r1, weather.rainProps.r2, weather.rainProps.r3, weather.rainProps.r4) : 0}%`,
    );
    lines.push(`-------------------`);
    baseSelectedStrategy.stints.forEach((s, idx) => {
      lines.push(
        `*S${idx + 1}:* ${s.laps} Laps | ${COMPOUND_FULL_NAMES[s.tyres]} | ⛽ ${s.fuelNeeded.toFixed(1)}L | 🔧 ${s.wearEnd.toFixed(0)}%`,
      );
    });
    lines.push(`-------------------`);
  ...
  const selectedStrategy = useMemo(() => {
    if (
      !baseSelectedStrategy ||
      !manualStintLaps ||
      manualStintLaps.length !== baseSelectedStrategy.stints.length
    )
      return baseSelectedStrategy;
    ...
    for (let i = 0; i < manualStintLaps.length; i++) {
      ...
      if (i < manualStintLaps.length - 1) {
        const pitTime =
          calcParams.track.pitTimeBase +
          simulateStint(
            manualStintLaps[i + 1],
            baseSelectedStrategy.stints[i + 1].tyres,
            calcParams,
            currentLap,
          ).fuelNeeded /
            calcParams.pitRefuelRate;
  ...
  const fuelGraphData = useMemo(() => {
    ...
    selectedStrategy.stints.forEach((stint, idx) => {
      ...
      for (let i = 0; i <= stint.laps; i++) {
        data.push({
          lap: currentLap + i,
          remaining: hiddenStints.includes(idx)
            ? null
            : parseFloat((stint.fuelStart - fuelPerLap * i).toFixed(2)),
          stint: `Stint ${idx + 1}`,
        });
      }
    ...
    selectedStrategy.stints.forEach((stint, idx) => {
      ...
      for (let i = 0; i <= stint.laps; i++) {
        data.push({
          ...
          remaining: hiddenStints.includes(idx)
            ? null
            : parseFloat((100 - wearPerLap * i).toFixed(2)),
  ...
  useEffect(() => {
    setSystemLogs((prev) => {
      const currentDynamicAlerts: {id: string; type: "in-game" | "patch-note"; title: string; message: string; timestamp: string;}[] = [];
      const triggeredIds: string[] = [];

      const addAlert = (id: string, title: string, message: string) => {
        triggeredIds.push(id);
        if (!settings.dismissedAlerts?.includes(id)) {
          currentDynamicAlerts.push({
            id,
            type: "in-game",
            title,
            message,
      ...
      if (timeLeft && timeLeft.d === 0) {
        addAlert("alert-upcoming-race", t("Upcoming Race Reminder"), t("The next race is starting within 24 hours. Ensure your strategy is ready."));
      }

      if ((player.league === "Rookie" || player.league === "Amateur") && player.riskAggression > 60) {
        addAlert("alert-high-risk", t("High Risk Strategy Warning"), t("Risk level is very high for your current league. This significantly increases mistake probability."));
      }

      if (weather.q1 === "Rain" || weather.rainProps > 0) {
        addAlert("alert-rain", t("Rain Expected"), t("Rain is expected. Do not forget to adjust your pit strategy for wet tyres."));
      }
      
      if (player.riskAggression > 50 && (weather.q1 === "Rain" || weather.rainProps > 0)) {
        addAlert("alert-extreme-risk-rain", t("Extreme Risk in Rain"), t("High risk in wet conditions significantly increases crash probability."));
      }

      if (activeTrack && activeTrack.wearFactor > 1.3) {
        addAlert("alert-high-wear", t("High Track Wear"), t("This track is hard on tyres. Consider harder compounds or shorter stints."));
      }
      
      if (player.driverStamina < 50) {
        addAlert("alert-low-stamina", t("Driver Fatigue Warning"), t("Driver stamina is critically low. Late-race mistakes are highly likely."));
      }
      
      if (weather.tempBase < 15) {
        addAlert("alert-cold-track", t("Cold Track Conditions"), t("Low track temperatures make it harder to warm up harder tyre compounds."));
      } else if (weather.tempBase > 35) {
        addAlert("alert-hot-track", t("Hot Track Conditions"), t("High track temperatures will accelerate tyre wear."));
      }
      
      if (player.baseFuelPerLap > 2.5) {
        addAlert("alert-high-fuel", t("High Fuel Consumption"), t("Your car has high fuel consumption. Check your fuel limits for longer stints."));
      }

      const dynamicIds = ["alert-upcoming-race", "alert-high-risk", "alert-rain", "alert-extreme-risk-rain", "alert-high-wear", "alert-low-stamina", "alert-cold-track", "alert-hot-track", "alert-high-fuel"];
      
      // If a condition is NO LONGER met, we should remove it from dismissedAlerts so it can trigger again later
      const staleDismissed = (settings.dismissedAlerts || []).filter(id => dynamicIds.includes(id) && !triggeredIds.includes(id));
      if (staleDismissed.length > 0) {
        // We use setTimeout to avoid state updates during render
        setTimeout(() => {
          setLocalSettings(s => ({
            ...s,
            dismissedAlerts: (s.dismissedAlerts || []).filter(id => !staleDismissed.includes(id))
          }));
        }, 0);
      }

      const baseLogs = prev.filter(l => !dynamicIds.includes(l.id));

      const newAlerts = currentDynamicAlerts.map(newAlert => {
        const existing = prev.find(l => l.id === newAlert.id);
        return existing || newAlert;
      });

      const merged = [...newAlerts, ...baseLogs].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
      
      if (JSON.stringify(merged.map(l => l.id)) === JSON.stringify(prev.map(l => l.id))) {
         return prev;
      }
      return merged;
    });
  }, [timeLeft?.d, player.league, player.riskAggression, weather.q1, weather.rainProps, activeTrack?.id, activeTrack?.wearFactor, settings.dismissedAlerts, setLocalSettings]);
  ...
  return (
    <div
      className={`min-h-screen font-sans antialiased selection:bg-indigo-100 flex flex-col max-lg:data-[landscape=true]:flex-row transition-colors ${"bg-slate-50/50 dark:bg-[#0a0f1c] text-slate-900 dark:text-slate-100"}`}
    >
      <header
        className={`backdrop-blur-md border-b dark:border-slate-700 max-lg:data-[landscape=true]:border-b-0 max-lg:data-[landscape=true]:border-r sticky top-0 z-[60] shadow-sm flex-none max-lg:data-[landscape=true]:h-screen max-lg:data-[landscape=true]:w-64 max-lg:data-[landscape=true]:overflow-y-auto ${"bg-white/80 dark:bg-[#0a0f1c]/80 backdrop-blur-xl border-slate-200/50 dark:border-slate-800/50"}`}
      >
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center py-3 gap-3 max-lg:data-[landscape=true]:flex-col max-lg:data-[landscape=true]:items-start max-lg:data-[landscape=true]:py-6 max-lg:data-[landscape=true]:h-full">
            <div className="flex items-center gap-3 w-full lg:w-auto justify-between max-lg:data-[landscape=true]:mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1
                    translate="no"
                    className="text-2xl font-display font-black tracking-tight flex items-baseline gap-1"
                  >
                    Pitwall <span className="text-indigo-600">Pro</span>{" "}
                    <span className="text-sm font-normal text-slate-400/70 dark:text-slate-500/70 tracking-normal ml-1">
                      d1.1.5
                    </span>
                  </h1>
                </div>
            ...
            {appView === "calculator" && (
              <div className="flex flex-row p-0.5 sm:p-1.5 rounded-2xl border dark:border-slate-800/80 w-full lg:w-auto shadow-inner relative z-10 transition-all gap-0.5 sm:gap-0 bg-slate-100/80 dark:bg-[#0f172a]/60 backdrop-blur-xl border-slate-200/60 justify-between sm:justify-start">
                ...
                >
                  <span className="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2">
                    <Goal className="w-4 h-4" /> {t("Strategy & Track")}
                  </span>
                </button>
                <button
                ...
                >
                  <span className="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2">
                    <User className="w-4 h-4" /> {t("Garage & Economy")}
                  </span>
                </button>
                <button
                ...
                >
                  <span className="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2">
                    <TrendingUp className="w-4 h-4" /> {t("Data & Assistant")}
                  </span>
                </button>
              </div>
            )}
            ...
            <div
              className={`hidden lg:flex items-center gap-3 max-lg:data-[landscape=true]:flex max-lg:data-[landscape=true]:mt-auto`}
            >
              <button
                onClick={toggleNotifications}
                className="p-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-white rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition relative shadow-sm border border-slate-200 dark:border-slate-700"
      ...
      <div className="flex-1 pb-24 pt-6 max-lg:data-[landscape=true]:h-screen max-lg:data-[landscape=true]:overflow-y-auto">
        <main className="max-w-7xl mx-auto px-4 lg:px-8 max-lg:data-[landscape=true]:px-6">
          {systemLogs.find((l) => l.id === "live-race-active") && (
            <div className="mb-6 bg-red-600 text-white rounded-2xl p-4 shadow-xl flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 animate-in fade-in zoom-in-95 relative overflow-hidden group border border-red-500">
              ...
              <div className="bg-red-700/50 p-2 rounded-full shrink-0 relative z-10 border border-red-500/50">
                <Activity className="w-6 h-6 animate-pulse text-red-100" />
              </div>
              <div className="relative z-10 text-center sm:text-left">
                <h3 className="font-bold text-base sm:text-lg uppercase tracking-wider text-white">
                  {t("Live Race Active!")}
                </h3>
                <p className="text-red-100 text-xs sm:text-sm mt-0.5 sm:mt-1 font-medium max-w-xl">
                  {t("We are currently within race hours. Jump into the race when you are ready to apply your strategies!")}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="max-w-3xl mx-auto animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-4 mb-6">
                <button
                  ...
                  className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:bg-slate-900 transition text-slate-600 dark:text-slate-400"
                >
              ...
              {appView === "menu" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-500 transition text-left flex flex-col gap-3 group"
                  >
                    <div className="p-3 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-xl w-max group-hover:scale-110 transition">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                        {t("Theme, language and notifications")}
                      </p>
                    </div>
                  </button>
                  <button
                    onClick={() => setAppView("account")}
                    className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-indigo-300 transition text-left flex flex-col gap-3 group"
                  >
                    <div className="p-3 bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded-xl w-max group-hover:scale-110 transition">
                  ...
                  <button
                    onClick={() => setAppView("simulation")}
                    className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-orange-300 dark:hover:border-orange-500 transition text-left flex flex-col gap-3 group"
                  >
                    <div className="p-3 bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 rounded-xl w-max group-hover:scale-110 transition">
                      <Activity className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-800 dark:text-slate-200 text-lg flex items-center gap-2">
                        Simulation Engine{" "}
                        <span className="px-1.5 py-0.5 text-[10px] bg-red-100 text-red-600 font-bold uppercase rounded-md tracking-wider">
                          Lab
                        </span>
                      </h3>
                      <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                        Test race conditions and strategies. Open to all for now
                  ...
                  <button
                    ...
                    className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-emerald-300 dark:hover:border-emerald-500 transition text-left flex flex-col gap-3 group"
                  >
                    <div className="p-3 bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-xl w-max group-hover:scale-110 transition">
                      <Activity className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-800 dark:text-slate-200 text-lg">
                  ...
                  {currentUser?.email === "burakis204@gmail.com" && (
                    <button
                      ...
                      className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-emerald-300 transition text-left flex flex-col gap-3 group"
                    >
                      <div className="p-3 bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-xl w-max group-hover:scale-110 transition">
                        <Activity className="w-6 h-6" />
                      </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-6 relative">
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      <button
                        onClick={() =>
                          setLocalSettings({ ...settings, theme: "light" })
                        }
                      ...
                      <button
                        onClick={() =>
                          setLocalSettings({ ...settings, theme: "dark" })
                        }
                      ...
                      <button
                        onClick={() =>
                          setLocalSettings({ ...settings, theme: "system" })
                        }
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-4 items-center flex gap-1">
                      Preferences <Tooltip content="Optional app behaviors" />
                    </h3>
                    <div className="space-y-3">
                      <label className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                        <div>
                          <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                            ...
                            <Tooltip content="Alerts for transitions" />
                          </div>
                          <div className="text-xs text-slate-500 dark:text-slate-400">
                            Pit time and wear alerts.
                          </div>
                        </div>
                        <input
                          type="checkbox"
                      ...
                      <div className="flex flex-col p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl transition">
                        ...
                        <select
                          ...
                          className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl p-2.5 text-sm transition-all focus:ring-2 focus:ring-indigo-500/50 outline-none"
                        >
                      ...
                      <label className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                        <div>
                          <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                            Smart Sync{" "}
                            <Tooltip content="Minimize cloud data transfer" />
                          </div>
                          <div className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <div className="mt-8 flex justify-end">
                    ...
                    >
                      <MessageSquare className="w-4 h-4" /> {t("Send Feedback")}
                    </button>
                  </div>
                </div>
              )}
              ...
              {appView === "account" && (
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800/80 shadow-sm flex flex-col items-center">
                  ...
                  <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">{t("Cloud Synchronization")}</h2>
                  <p className="text-slate-500 dark:text-slate-400 mt-2 text-center max-w-md">{t("Backup your strategy and car data to the cloud.")}</p>

                  <div className="mt-8 w-full max-w-md">
                    {!currentUser ? (
                      <div className="flex flex-col gap-4 animate-fade-in w-full">
                        <div className="text-center mb-4">
                          <h3 className="text-2xl font-display font-black tracking-tight text-slate-800 dark:text-slate-100">
                            {authMode === "login" ? t("Login") : authMode === "register" ? t("Register") : t("Reset Password")}
                          </h3>
                        </div>

                        {authError && (
                          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-300 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2">
                            <svg
                        ...
                        {authSuccess && (
                          <div className="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-300 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2">
                            <svg
                              className="w-5 h-5 shrink-0"
                              fill="none"
                              viewBox="0 0 24 24"
                        ...
                        <form
                          ...
                          onSubmit={(e: any) => {
                            ...
                            if (authMode === "reset") {
                              if (!email) {
                                setAuthError("Lütfen email adresinizi girin.");
                                return;
                              }
                            ...
                            if (!email || !password) {
                              setAuthError(
                                t("Please fill in email and password."),
                              );
                              return;
                            }
                            ...
                            if (authMode === "register") {
                              createUserWithEmailAndPassword(
                                ...
                                .catch((err) => {
                                  ...
                                  if (err.code === "auth/email-already-in-use")
                                    msg = t("This email is already in use.");
                                  else if (err.code === "auth/weak-password")
                                    msg =
                                      t("Password is too weak (min 6 characters).");
                                  else if (err.code === "auth/invalid-email")
                                    msg = t("Invalid email format.");
                                  setAuthError(msg);
                                });
                              ...
                              signInWithEmailAndPassword(auth, email, password)
                                .then(() => setAuthSuccess(t("Login successful!")))
                                .catch((err) => {
                                  let msg = err.message;
                                  if (
                                    ...
                                    msg = t("Invalid email or password.");
                                  else if (
                        ...
                        >
                          <div className="space-y-3">
                            <div className="relative">
                              <input
                                ...
                                className="text-slate-800 dark:text-slate-100 w-full pl-10 pr-4 py-3.5 bg-slate-50 dark:bg-[#0a0f1c] border border-slate-200 dark:border-slate-800/80 rounded-xl focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all placeholder:text-slate-400 text-sm font-medium"
                                required
                              />
                          ...
                          <div className="mt-4">
                            <button
                              type="submit"
                              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 active:translate-y-0 text-sm"
                            >
                              {authMode === "login" ? t("Login") : authMode === "register" ? t("Register") : t("Send Password Reset Link")}
                            </button>
                          </div>
                            ...
                            {authMode === "login" ? (
                              <>
                                <button
                                  ...
                                  className="text-xs text-slate-500 dark:text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 font-medium transition-colors"
                                >
                        ...
                        <div className="relative w-full flex items-center justify-center my-4">
                          ...
                          <div className="relative bg-white dark:bg-slate-900 px-4 text-[10px] font-bold text-slate-400 tracking-widest uppercase">
                            VEYA
                          </div>
                        </div>
                        <button
                          onClick={() =>
                            signInWithPopup(
                              ...
                              if (err.code !== "auth/cancelled-popup-request")
                                setAuthError(
                                  "Google Login Hatası: " + err.message,
                                );
                            })
                          }
                        ...
                        >
                          ...
                          <span className="text-sm">{t("Sign in with Google")}</span>
                        </button>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-4 w-full">
                        <div className="flex items-center gap-3 w-full bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                          <div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold text-lg uppercase line-clamp-1">
                            {currentUser.email?.[0]}
                          </div>
                          <div
                          ...
                          >
                            <div className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">
                              {currentUser.displayName || "GPRO Player"}
                            </div>
                          ...
                          <button
                            ...
                            className="text-xs text-red-500 hover:text-red-700 font-bold uppercase transition px-2"
                          >
                            {t("Logout")}
                          </button>
                        </div>
                        ...
                        <div className="w-full space-y-3 mt-4 flex flex-col items-center">
                          <div className="text-center text-sm font-bold text-slate-400 p-4 border border-dashed border-slate-200 dark:border-slate-700 rounded-xl w-full">
                            Ayarlarınız ve profiliniz başarıyla senkronize
                            ediliyor.
                          </div>
                          {syncStatus && (
                            <div className="text-xs text-center font-medium text-emerald-600 bg-emerald-50 py-2 w-full rounded-lg border dark:border-slate-700 border-emerald-100">
                              {syncStatus}
                            </div>
                          )}
                        </div>
          ...
          {appView === "calculator" && (
            <React.Fragment>
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-end w-full md:w-auto">
                    ...
                    <button
                      onClick={() =>
                        setLocalSettings((s) => ({
                          ...s,
                          isPrepOverlayActive: !s.isPrepOverlayActive,
                        }))
                      }
                    ...
                    <div className="bg-white dark:bg-gradient-to-r dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white shadow-sm dark:shadow-lg rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-3 relative z-10 w-full sm:w-auto">
                        ...
                        <div className="min-w-0 pr-4">
                          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-0.5 whitespace-nowrap">
                            {t("Next Race")}
                          </p>
                          <p className="font-black text-lg text-slate-900 dark:text-white truncate max-w-[200px] sm:max-w-xs">
                            {nextRaceName
                              ? t(nextRaceName)
                              : t("Pending") + "..."}
                          </p>
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-4 relative z-10 w-full sm:w-auto">
                        ...
                        >
                          ...
                          {t("Start New Race Prep")}
                        </button>
                        <div className="flex items-center justify-center gap-1.5 sm:gap-2 font-mono font-bold text-xl sm:text-2xl">
                          <div className="flex flex-col items-center bg-slate-100 dark:bg-black/40 backdrop-blur-md px-1.5 sm:px-2 py-1.5 rounded-xl w-12 sm:w-16 border border-slate-200 dark:border-white/5">
                            <span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="rounded-2xl p-6 bg-indigo-600 text-white shadow-lg relative overflow-hidden">
                      ...
                      <p className="text-indigo-100 font-medium">
                        {activeTrack.laps} {t("Laps")} • {activeTrack.distance}{" "}
                        km
                      </p>
                      <button
                        onClick={() => setActiveTab("track_setup")}
                        className="mt-6 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl text-sm font-bold transition backdrop-blur-md"
                      >
                        Change Track
                      </button>
                    </div>
                    ...
                    <div className="rounded-2xl p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-4 opacity-5 dark:opacity-10 text-slate-500 dark:text-slate-100">
                        <User className="w-24 h-24" />
                      </div>
                      <h3 className="text-xl font-bold mb-2 text-slate-500 dark:text-slate-400">
                        {t("Active Driver")}
                      </h3>
                      <p className="text-3xl font-black mb-1 text-slate-800 dark:text-slate-100">
                        {player.name || "Driver 1"}
                      </p>
                      <p className="text-slate-600 dark:text-slate-400 font-medium">
                        Aggression: {player.riskAggression} • Exp:{" "}
                        {player.driverExperience}
                      </p>
                      <button
                        onClick={() => setActiveTab("driver_profile")}
                        className="mt-6 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-sm font-bold transition border border-slate-200 dark:border-slate-700/50"
                      >
                        Configure Driver
                      </button>
                    </div>
                    ...
                    <div className="rounded-2xl p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between">
                      ...
                      <button
                        onClick={() => setActiveTab("strategy")}
                        className="mt-4 px-4 py-2 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 rounded-xl text-sm font-bold transition w-fit"
                      >
                        View Strategies
                      </button>
                    </div>
                    ...
                    <div className="rounded-2xl p-6 bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-emerald-700 dark:text-emerald-400 text-sm font-medium mb-1">
                          {t("Race Analysis & Pre-Race Checklist")}
                        </p>
                        <p className="text-emerald-600/80 dark:text-emerald-500/80 text-xs">
                          {t("Calibration and test lap evaluator.")}
                        </p>
                      </div>
                      <button
                        onClick={() => setActiveTab("assistant")}
                        className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold transition w-fit flex items-center gap-2"
                      >
                        {t("Open Assistant")}{" "}
                        ...
                        <p className="text-indigo-800/80 dark:text-indigo-300/80 text-sm leading-relaxed">
                          {
                            [
                              ...
                              t(
                                "Fuel weight penalty is around 0.03s per liter. Adding too much extra fuel will slow you down significantly.",
                              ),
                              t(
                                "When temperatures exceed 25°C, tyre wear increases rapidly. Harder compounds become much more favorable.",
                              ),
                              t(
                                "If a component's wear exceeds 90% during the race, the DNF risk skyrockets. Always keep an eye on parts wear!",
                              ),
                              t(
                                "Aggressive driving style improves lap times but increases fuel consumption and part wear. Find the perfect balance.",
                              ),
                            ][new Date().getDay() % 5]
                          }
                        </p>
                    ...
                    <section className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                        <h2 className="text-lg font-semibold flex items-center gap-2 text-indigo-900 dark:text-indigo-400">
                          <MapIcon className="w-5 h-5 text-indigo-500" /> Track
                          & Conditions
                        </h2>
                        <div className="flex items-center gap-2 sm:self-auto self-end">
                          <button
                            onClick={() => guessCurrentTrack(false)}
                            className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                          >
                            🌐 {t("Fetch F1 Calendar")}
                          </button>
                        </div>
                      ...
                      <div className="space-y-5">
                        <div>
                          <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2 relative w-max pr-3">
                            {t("Track Selection")}{" "}
                            <PrepDot
                              fieldKey="track"
                              active={settings.isPrepOverlayActive}
                              settings={settings}
                            />
                          </label>
                          <select
                          ...
                          <div className="grid grid-cols-2 gap-4 p-5 bg-blue-50/50 dark:bg-slate-800/30 rounded-xl border dark:border-slate-700/50 border-blue-100">
                            <div className="col-span-2">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 flex items-center gap-1.5">
                                Laps <Tooltip content="Total number of laps." />
                              </label>
                              <NumberInput
                                value={customTrack.laps}
                                onChange={(val) =>
                                  setCustomTrack({ ...customTrack, laps: val })
                                }
                                className="w-full p-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-lg sm:text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                              />
                            </div>
                            <div>
                              ...
                              <select
                                value={customTrack.wearSeverity}
                                onChange={(e) =>
                                  setCustomTrack({
                                    ...customTrack,
                                    wearSeverity: e.target.value as any,
                                  })
                                }
                                className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl p-2.5 text-sm transition-all focus:ring-2 focus:ring-indigo-500/50 outline-none"
                              >
                                {Object.keys(SEVERITY_MULTIPLIERS).map((k) => (
                            ...
                            <div>
                              ...
                              <select
                                value={customTrack.fuelSeverity}
                                onChange={(e) =>
                                  setCustomTrack({
                                    ...customTrack,
                                    fuelSeverity: e.target.value as any,
                                  })
                                }
                                className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl p-2.5 text-sm transition-all focus:ring-2 focus:ring-indigo-500/50 outline-none"
                              >
                            ...
                            <div className="col-span-2">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 flex items-center gap-1.5">
                                Base Pit Time (s){" "}
                                <Tooltip content="Time lost in pitlane without refuelling (usually 20-24s)." />
                              </label>
                              <NumberInput
                                value={customTrack.pitTimeBase}
                                onChange={(val) =>
                                  setCustomTrack({
                                    ...customTrack,
                                    pitTimeBase: val,
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                          ...
                          <div className="grid grid-cols-2 gap-4 mb-5">
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border dark:border-slate-700/50 border-slate-200 shadow-sm flex flex-col items-center">
                              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 text-center uppercase tracking-wider relative w-max pr-3">
                                Min (°C)
                                <PrepDot
                                  fieldKey="weather"
                                  active={settings.isPrepOverlayActive}
                              ...
                              <div className="flex items-center justify-center w-full gap-3 mt-1">
                                <button
                                  onClick={() =>
                                    setWeather({
                                      ...weather,
                                      tempBase: weather.tempBase - 1,
                                    })
                                  }
                                  className="w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-700 rounded shadow-sm border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-300 hover:text-indigo-500 dark:hover:text-indigo-400 hover:border-indigo-200 transition font-bold"
                                  title="-1°C"
                                >
                                  -
                                </button>
                                <NumberInput
                                  value={weather.tempBase}
                                  onChange={(val) =>
                                    setWeather({ ...weather, tempBase: val })
                                  }
                                  onConfirm={() => markFieldUpdated("weather")}
                                  className="w-16 text-center bg-transparent dark:text-slate-100 text-2xl font-mono font-black text-slate-700 dark:text-slate-200 focus:outline-none"
                                />
                                <button
                                  onClick={() =>
                                    setWeather({
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border dark:border-slate-700/50 border-slate-200 shadow-sm flex flex-col items-center">
                              ...
                              <div className="flex items-center justify-center w-full gap-3 mt-1">
                                ...
                                <NumberInput
                                  ...
                                  onChange={(val) =>
                                    setWeather({ ...weather, tempMax: val })
                                  }
                                  onConfirm={() => markFieldUpdated("weather")}
                                  className="w-16 text-center bg-transparent dark:text-slate-100 text-2xl font-mono font-black text-slate-700 dark:text-slate-200 focus:outline-none"
                                />
                          ...
                          <div>
                            <h4 className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-3 flex items-center gap-1.5">
                              ...
                              {t("Rain Probabilities")} (%)
                              <Tooltip
                                content={t(
                                  "Configure qualifying and race rain probabilities.",
                                )}
                              />
                            </h4>
                            <div className="grid grid-cols-5 gap-2">
                              <div className="bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-lg border dark:border-slate-700/50 border-slate-200">
                                <label className="block text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 text-center mb-1.5">
                                  Q1
                                </label>
                                <NumberInput
                                  min={0}
                                  max={100}
                                  value={weather.rainProps.q1}
                                  onChange={(val) =>
                                    setWeather({
                                      ...weather,
                                      rainProps: {
                                  ...
                                  onConfirm={() => markFieldUpdated("weather")}
                                  className="w-full text-center bg-transparent dark:text-slate-100 text-sm font-mono font-bold text-blue-600 dark:text-blue-400 focus:outline-none"
                                />
                              </div>
                              <div className="bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-lg border dark:border-slate-700/50 border-slate-200">
                                <label className="block text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 text-center mb-1.5 break-words">
                                  Q2 & 0-30
                                </label>
                                <NumberInput
                                  min={0}
                                  max={100}
                                  value={weather.rainProps.q2_r1}
                                  onChange={(val) =>
                                    setWeather({
                                      ...weather,
                                      rainProps: {
                  ...
                  <div className="space-y-6">
                    <section className="bg-gradient-to-br from-blue-50 via-white to-indigo-50/40 p-6 rounded-2xl shadow-sm border dark:border-slate-700 border-blue-100">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 tracking-tight">
                              {t("Smart Setup Assistant")}
                            </h2>
                            <Tooltip
                              content={t(
                                "These suggestions are specifically estimated based on the track characteristics and air temperature.",
                              )}
                            />
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border dark:border-slate-700 border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center relative">
                          <div className="absolute top-2 right-2">
                            <Tooltip content="High wings for corner-heavy tracks, low wings for straights." />
                          </div>
                          ...
                          <span className="text-[10px] text-slate-400 mt-1">
                            {t("Increase downforce in high temperature")}
                          </span>
                        </div>
                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border dark:border-slate-700 border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center relative">
                          <div className="absolute top-2 right-2">
                            <Tooltip content="Gear ratios determine acceleration (short) or top speed (long)." />
                          </div>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                            Gearbox
                          </span>
                          ...
                          <span className="text-[10px] text-slate-400 mt-1">
                            {t("Acceleration focus")} ({activeTrack.name})
                          </span>
                        </div>
                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border dark:border-slate-700 border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center relative">
                          <div className="absolute top-2 right-2">
                            <Tooltip content="Engine cooling. Must be high in hot temperatures to avoid failure." />
                          </div>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                            {t("Engine")}
                          </span>
                          ...
                          <span className="text-[10px] text-slate-400 mt-1">
                            {weather.tempMax > 25
                              ? "Max temp is high, careful with engine"
                              : "Normal revs"}
                          </span>
                        </div>
                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border dark:border-slate-700 border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center relative">
                          <div className="absolute top-2 right-2">
                            <Tooltip content="Chassis stiffness and curb use. Directly linked to driver skills." />
                          </div>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                            {t("Suspension")}
                          </span>
                      ...
                      <div className="mt-5 p-4 bg-blue-100/50 rounded-xl text-sm text-blue-900 border dark:border-slate-700 border-blue-200 border-dashed flex items-start gap-3">
                        ...
                        <p>
                          <strong>{t("Setup Note:")}</strong> Calculations
                          automatically adapt using the temperature{" "}
                          <span className="font-mono bg-blue-200 px-1 rounded">
                            {((weather.tempBase + weather.tempMax) / 2).toFixed(
                              1,
                            )}
                            °C
                          </span>{" "}
                          ...
                          data and adapts accordingly. Driver skills affect
                          suspension travel.
                        </p>
                      </div>
                    </section>
                    ...
                    <section className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <button
                            onClick={exportProfileToClipboard}
                            className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                            title={t("Copy Profile ID ID to Clipboard")}
                          >
                            <User className="w-3.5 h-3.5" /> Copy ID
                          </button>
                          <button
                            onClick={importProfileFromClipboard}
                            className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                            title={t("Load Profile ID from Clipboard")}
                          >
                            <User className="w-3.5 h-3.5" /> Load ID
                          </button>
                          <button
                            onClick={exportProfileToFile}
                            className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                            title={t("Save Profile as JSON File")}
                          >
                            ⬇️ Save
                          </button>
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                            title={t("Load Profile from JSON File")}
                          >
                            ⬆️ Load
                          </button>
                          <button
                            onClick={fetchGproApi}
                            className="px-3 py-1.5 bg-indigo-50 border border-indigo-200 dark:border-indigo-800/30 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                          >
                            <MonitorSmartphone className="w-3.5 h-3.5" /> API
                            Sync
                          </button>
                          <button
                            onClick={loadDriverFromCloud}
                            className="px-3 py-1.5 bg-blue-50 border border-blue-200 dark:border-blue-800/30 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                          >
                            <CloudCog className="w-3.5 h-3.5" /> Load Cloud
                          </button>
                          <button
                            onClick={saveDriverToCloud}
                            className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 dark:border-emerald-800/30 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition flex items-center gap-1.5 text-[11px] font-bold shadow-sm"
                          >
                            <Save className="w-3.5 h-3.5" /> Save Cloud
                          </button>
                          <button
                            onClick={resetPlayerSettings}
                            className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 border border-transparent rounded-full text-slate-400 hover:text-slate-600 dark:text-slate-400 transition flex items-center justify-center text-[10px] font-bold uppercase tracking-widest"
                          >
                            <RotateCcw className="w-4 h-4" />
                            <Tooltip content="Reset values to default." />
                          </button>
                        </div>
                      </div>
                      <div className="space-y-4">
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
                              {t("Tyre Supplier")}{" "}
                              <Tooltip content="Different brands have different wear rates." />
                            </label>
                            <button
                              onClick={() =>
                                setPlayer({
                              ...
                              className="text-slate-400 hover:text-indigo-500 transition border dark:border-slate-700 bg-white dark:bg-slate-800 p-1 rounded shadow-sm"
                            >
                              <RotateCcw className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          <select
                            onBlur={() => markFieldUpdated("tyreSupplier")}
                            value={player.tyreSupplier}
                            onChange={(e) =>
                              setPlayer({
                                ...player,
                                tyreSupplier: e.target.value,
                              })
                            }
                            className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl p-2.5 text-sm transition-all focus:ring-2 focus:ring-indigo-500/50 outline-none"
                          >
                            <option value="Pipirelli">
                              Pipirelli ({t("Fast / Very High Wear")})
                            </option>
                            <option value="Avonn">
                              Avonn ({t("Extreme Wear")})
                            </option>
                            <option value="Contimental">
                              Contimental ({t("Balanced")})
                            </option>
                            <option value="Dunnolop">
                              Dunnolop ({t("Durable")})
                            </option>
                            <option value="Yokohama">
                              Yokohama ({t("Highly Durable")})
                            </option>
                            <option value="Michelin">
                              Michelin ({t("Extremely Durable")})
                            </option>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
                              {t("Base Fuel Burn")}{" "}
                              <Tooltip content="Standard fuel burn per lap. (3.0 L/t is default)." />
                            </label>
                            <div className="flex items-center gap-3">
                              <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                {player.baseFuelPerLap} L/t
                              </span>
                              <button
                                onClick={() =>
                                  setPlayer({ ...player, baseFuelPerLap: 3.0 })
                                }
                                className="text-slate-400 hover:text-indigo-500 transition border dark:border-slate-700 bg-white dark:bg-slate-800 p-1 rounded shadow-sm"
                              >
                                <RotateCcw className="w-3.5 h-3.5" />
                          ...
                          <input
                            type="range"
                            onMouseUp={() => markFieldUpdated("baseFuelPerLap")}
                            min="1.5"
                            max="5.0"
                            step="0.1"
                            value={player.baseFuelPerLap}
                            onChange={(e) =>
                              setPlayer({
                                ...player,
                                baseFuelPerLap: Number(e.target.value),
                              })
                            }
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
                              {t("Tyre Wear Multiplier")}{" "}
                              <span className="relative w-max pl-2">
                                <PrepDot
                                  fieldKey="driverWearMultiplier"
                                  active={settings.isPrepOverlayActive}
                                  settings={settings}
                                />
                              </span>
                              <Tooltip content="Hidden risk multiplier added to wear calculations." />
                            </label>
                            <div className="flex items-center gap-3">
                              <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                x{player.baseWearMultiplier.toFixed(2)}
                              </span>
                              <button
                                onClick={() =>
                                  setPlayer({
                          ...
                          <input
                            ...
                            max="1.5"
                            step="0.05"
                            value={player.baseWearMultiplier}
                            onChange={(e) =>
                              setPlayer({
                                ...player,
                                baseWearMultiplier: Number(e.target.value),
                              })
                            }
                            className="w-full accent-indigo-500"
                          />
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
                              ...
                              <span className="relative w-max pl-2">
                                <PrepDot
                                  fieldKey="riskAggression"
                                  active={settings.isPrepOverlayActive}
                                  settings={settings}
                                />
                              </span>
                              <Tooltip content="Driver's standard in-race aggression (20 is default)." />
                            </label>
                            <div className="flex items-center gap-3">
                              <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                {player.riskAggression}
                              </span>
                              <button
                                onClick={() =>
                                  setPlayer({ ...player, riskAggression: 20 })
                                }
                                className="text-slate-400 hover:text-indigo-500 transition border dark:border-slate-700 bg-white dark:bg-slate-800 p-1 rounded shadow-sm"
                              >
                                <RotateCcw className="w-3.5 h-3.5" />
                          ...
                          <input
                            ...
                            max="100"
                            step="1"
                            value={player.riskAggression}
                            onChange={(e) =>
                              setPlayer({
                                ...player,
                                riskAggression: Number(e.target.value),
                              })
                            }
                            ...
                            <button
                              onClick={() =>
                                setPlayer({
                                  ...player,
                                  riskAggression: 5,
                                  baseWearMultiplier: 0.95,
                                })
                              }
                              className="flex-1 text-[10px] font-bold uppercase tracking-wider py-1.5 rounded bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800 hover:bg-emerald-100 transition whitespace-nowrap overflow-hidden text-ellipsis px-1"
                            >
                              {t("Safe")}
                            </button>
                            <button
                              onClick={() =>
                                setPlayer({
                                  ...player,
                                  riskAggression: 35,
                                  baseWearMultiplier: 1.05,
                                })
                              }
                              className="flex-1 text-[10px] font-bold uppercase tracking-wider py-1.5 rounded bg-indigo-50 text-indigo-600 border border-indigo-200 dark:bg-indigo-900/30 dark:border-indigo-800 hover:bg-indigo-100 transition whitespace-nowrap overflow-hidden text-ellipsis px-1"
                            >
                              {t("Balanced")}
                            </button>
                            <button
                              onClick={() =>
                                setPlayer({
                                  ...player,
                                  riskAggression: 80,
                                  baseWearMultiplier: 1.25,
                                })
                              }
                              className="flex-1 text-[10px] font-bold uppercase tracking-wider py-1.5 rounded bg-red-50 text-red-600 border border-red-200 dark:bg-red-900/30 dark:border-red-800 hover:bg-red-100 transition whitespace-nowrap overflow-hidden text-ellipsis px-1"
                            >
                              {t("Aggressive")}
                            </button>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-6 gap-y-4 border-t pt-5 border-slate-100 dark:border-slate-800 mt-2">
                          <div className="col-span-2">
                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-2 flex items-center gap-1.5">
                              {t("Risk Distribution")}{" "}
                              <Tooltip content="Set specific risks for the engine. Determines pace and mistake chance." />
                            </label>
                              ...
                              {[
                                ...
                                <div key={k}>
                                  <div className="flex justify-between items-center text-xs mb-1 text-slate-500 dark:text-slate-400">
                                    <span className="relative w-max pr-3 capitalize">
                                      {t(k)}
                                      <PrepDot
                                        fieldKey="risks"
                                        active={settings.isPrepOverlayActive}
                                    ...
                                    <span className="font-mono text-indigo-500 font-bold">
                                      {player.risks?.[
                                        k as keyof typeof player.risks
                                      ] || 0}
                                    </span>
                                  </div>
                                  <input
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                ...
                                <span className="relative w-max pl-2">
                                  <PrepDot
                                    fieldKey="driverFocus"
                                    active={settings.isPrepOverlayActive}
                                    settings={settings}
                                  />
                                </span>
                                <Tooltip content="Default: 100" />
                              </label>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                  {player.driverFocus}
                                ...
                                <button
                                  onClick={() =>
                                    setPlayer({ ...player, driverFocus: 100 })
                                  }
                            ...
                            <input
                              type="range"
                              onMouseUp={() => markFieldUpdated("driverFocus")}
                              min="0"
                              max="250"
                              value={player.driverFocus}
                              onChange={(e) =>
                                setPlayer({
                                  ...player,
                                  driverFocus: Number(e.target.value),
                                })
                              }
                              className="w-full accent-indigo-500"
                            />
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                  {player.driverStamina}
                                </span>
                                <button
                                  onClick={() =>
                                    setPlayer({ ...player, driverStamina: 100 })
                                  }
                                  className="text-slate-400 hover:text-indigo-500 transition"
                                >
                                  <RotateCcw className="w-3.5 h-3.5" />
                            ...
                            <input
                              type="range"
                              onMouseUp={() =>
                                markFieldUpdated("driverStamina")
                              }
                              min="0"
                              max="250"
                              value={player.driverStamina}
                              onChange={(e) =>
                                setPlayer({
                                  ...player,
                                  driverStamina: Number(e.target.value),
                                })
                              }
                              className="w-full accent-indigo-500"
                            />
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                ...
                                <span className="relative w-max pl-2">
                                  <PrepDot
                                    fieldKey="driverExperience"
                                    active={settings.isPrepOverlayActive}
                                    settings={settings}
                                  />
                                </span>
                                <Tooltip content="Default: 50" />
                              </label>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                                  {player.driverExperience}
                            ...
                            <input
                              type="range"
                              onMouseUp={() =>
                                markFieldUpdated("driverExperience")
                              }
                              min="0"
                              max="250"
                              value={player.driverExperience}
                              onChange={(e) =>
                                setPlayer({
                                  ...player,
                                  driverExperience: Number(e.target.value),
                                })
                              }
                              className="w-full accent-indigo-500"
                            />
                          ...
                          <div className="flex flex-col justify-end">
                            ...
                            <div className="flex gap-1 h-[36px]">
                              <button
                                ...
                                className="flex-1 bg-slate-100 dark:bg-slate-800 text-[10px] whitespace-nowrap overflow-hidden font-bold rounded text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition"
                                title="Rookie Driver"
                              >
                                Rook
                              </button>
                              ...
                              <button
                                onClick={() =>
                                  setPlayer({
                                    ...
                                    driverFocus: 250,
                                    driverStamina: 250,
                                    driverExperience: 250,
                                  })
                                }
                                className="flex-1 bg-slate-100 dark:bg-slate-800 text-[10px] whitespace-nowrap overflow-hidden font-bold rounded text-slate-600 dark:text-slate-400 hover:bg-indigo-100 hover:text-indigo-600 transition"
                                title="Maxed Driver"
                              >
                                Max
                              </button>
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1.5">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                ...
                                <Tooltip content="League you play in." />
                              </label>
                            </div>
                            <select
                              value={player.league || "Rookie"}
                              onChange={(e) =>
                                setPlayer({
                                  ...player,
                                  league: e.target.value as any,
                                })
                              }
                              className="text-slate-800 dark:text-slate-100 w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl p-2.5 text-sm transition-all focus:ring-2 focus:ring-indigo-500/50 outline-none"
                            >
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1.5">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                ...
                                <span className="relative w-max pl-2">
                                  <PrepDot
                                    fieldKey="startPosition"
                                    active={settings.isPrepOverlayActive}
                                    settings={settings}
                                  />
                                </span>
                                <Tooltip content="Penalizes pit stops more if you start at the front." />
                              </label>
                            </div>
                            <NumberInput
                              min={1}
                              max={player.totalRacers || 40}
                              value={player.startPosition || 1}
                              onChange={(val) =>
                                setPlayer({ ...player, startPosition: val })
                              }
                              onConfirm={() =>
                                markFieldUpdated("startPosition")
                              }
                              className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-lg sm:text-sm focus:ring-1 focus:ring-indigo-500"
                            />
                          </div>
                          <div>
                            <div className="flex items-center justify-between mb-1.5">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                ...
                                <span className="relative w-max pl-2">
                                  <PrepDot
                                    fieldKey="totalRacers"
                                    active={settings.isPrepOverlayActive}
                                    settings={settings}
                                  />
                                </span>
                                <Tooltip content="Number of drivers in the grid." />
                              </label>
                            </div>
                            <NumberInput
                              min={1}
                              max={40}
                              value={player.totalRacers || 40}
                              onChange={(val) =>
                                setPlayer({ ...player, totalRacers: val })
                              }
                              onConfirm={() => markFieldUpdated("totalRacers")}
                              className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-lg sm:text-sm focus:ring-1 focus:ring-indigo-500"
                            />
                          </div>
                        </div>
                    ...
                    <section className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60">
                      <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
                        <h2 className="text-lg font-semibold flex items-center gap-2 text-indigo-900 dark:text-indigo-400">
                          ...
                          {t("Car Character (Quick Setup)")}
                        </h2>
                      </div>
                      <p className="text-xs text-slate-500 mb-4">
                        {t(
                          "Enter your current car character (Power, Handling, Acceleration). This is faster than entering all 11 parts and improves calculation stability.",
                        )}
                      </p>
                      <div className="grid grid-cols-3 gap-3">
                        {["power", "handling", "acceleration"].map((part) => (
                          <div key={part}>
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 capitalize relative w-max pr-3">
                              {t(part)}
                              <PrepDot
                                fieldKey="carPHA"
                                active={settings.isPrepOverlayActive}
                            ...
                            <NumberInput
                              ...
                              max={250}
                              value={
                                player.pha?.[part as keyof typeof player.pha] ||
                                0
                              }
                              onChange={(val) =>
                                setPlayer({
                    ...
                    <section className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60">
                      <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
                        <h2 className="text-lg font-semibold flex items-center gap-2 text-indigo-900 dark:text-indigo-400">
                          ...
                          {t("Advanced Constants")}
                        </h2>
                      </div>
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1 flex items-center relative w-max pr-3">
                            {t("Driver Weight (kg)")}
                            <PrepDot
                              fieldKey="driverWeight"
                              active={settings.isPrepOverlayActive}
                              settings={settings}
                            />
                          </label>
                          <NumberInput
                            value={constants.driverWeight}
                            onChange={(val) =>
                              setConstants({ ...constants, driverWeight: val })
                            }
                            onConfirm={() => markFieldUpdated("driverWeight")}
                            className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 dark:border-slate-700 border-slate-200 dark:border-slate-700 rounded-lg"
                          />
                          <p className="text-[10px] text-slate-400 mt-1">
                            {t("Calculated penalty per liter:")}{" "}
                            {(constants.driverWeight * 0.00045).toFixed(3)}s/L
                          </p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1 flex items-center relative w-max pr-3">
                            {t("Refuel Rate (L/s)")}
                            <PrepDot
                              fieldKey="pitRefuelRate"
                              active={settings.isPrepOverlayActive}
                              settings={settings}
                            />
                          </label>
                          <NumberInput
                            value={constants.pitRefuelRate}
                            onChange={(val) =>
                              setConstants({ ...constants, pitRefuelRate: val })
                            }
                            onConfirm={() => markFieldUpdated("pitRefuelRate")}
                            className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 dark:border-slate-700 border-slate-200 dark:border-slate-700 rounded-lg"
                          />
                        </div>
                      </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 max-lg:data-[landscape=true]:grid-cols-3 gap-4">
                    <div className="bg-blue-600 text-white p-5 rounded-2xl shadow-sm flex flex-col justify-between">
                      ...
                      <div>
                        ...
                        <p className="text-blue-100 text-sm mt-1">
                          {activeTrack.laps} Tur • {activeTrack.distance} km
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 p-5 rounded-2xl shadow-sm flex flex-col justify-between">
                      ...
                      <div>
                        <p className="text-2xl font-semibold tracking-tight text-slate-800 dark:text-slate-200">
                          {estFuelPerLap.toFixed(2)} L
                        </p>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                          ({activeTrack.fuelSeverity} karakteristik)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 p-5 rounded-2xl shadow-sm flex flex-col justify-between">
                      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-2">
                        ...
                        <span className="text-sm font-medium">
                          {t("Pit Stop Parameter")}
                        </span>
                      </div>
                      <div>
                        <p className="text-slate-800 dark:text-slate-200 text-lg font-medium">
                          {t("Base:")} {activeTrack.pitTimeBase} {t("sec")}
                        </p>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                          +{constants.pitRefuelRate} L/sn Added to refuel
                          duration
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl p-3 flex flex-wrap items-center justify-between gap-4 shadow-sm text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-800 dark:text-slate-200">
                            {weather.tempBase}°C - {weather.tempMax}°C
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700 overflow-hidden text-[10px] font-mono font-bold">
                        <div
                          className="px-1.5 border-r border-slate-100 dark:border-slate-800 py-1"
                          title="Q1"
                        >
                          Q1: {weather.rainProps?.q1}%
                        </div>
                        <div
                          className="px-1.5 border-r border-slate-100 dark:border-slate-800 py-1"
                          title="Q2/R1"
                        >
                          R1: {weather.rainProps?.q2_r1}%
                        </div>
                        <div
                          className="px-1.5 border-r border-slate-100 dark:border-slate-800 py-1"
                          title="R2"
                        >
                          R2: {weather.rainProps?.r2}%
                        </div>
                        <div
                          className="px-1.5 border-r border-slate-100 dark:border-slate-800 py-1"
                          title="R3"
                        >
                          R3: {weather.rainProps?.r3}%
                        </div>
                        <div className="px-1.5 py-1" title="R4">
                          R4: {weather.rainProps?.r4}%
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700/80 p-6 xl:p-8 flex-1">
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-6 flex items-center gap-3 border-b dark:border-slate-700 pb-4">
                      <BarChart3 className="w-6 h-6 text-emerald-500" /> Optimum
                      Strateji Analizi
                    </h2>

                    {results.length === 0 ? (
                      <div className="text-center py-12 text-slate-500 dark:text-slate-400">
                        No valid strategy found. Tyres might be overheating or
                        wearing too fast. Try softening the temp/wear
                        conditions.
                      </div>
                    ) : (
                      <div className="space-y-8">
                          ...
                          >
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                              <div>
                                <div className="flex flex-wrap items-center gap-2">
                                  <h3
                                    className={`text-lg font-bold ${selectedStratIndex === 0 ? "text-emerald-800 dark:text-emerald-300" : "text-indigo-900 dark:text-indigo-300"}`}
                                  >
                                    {t("Selected Strategy:")}{" "}
                                    {selectedStrategy!.stops} Pit Stop
                                  </h3>
                                  {manualStintLaps &&
                                    baseSelectedStrategy &&
                                    JSON.stringify(manualStintLaps) !==
                                      JSON.stringify(
                                        baseSelectedStrategy.stints.map(
                                          (s) => s.laps,
                                        ),
                                      ) && (
                                      <button
                                ...
                                <p
                                  className={`${selectedStratIndex === 0 ? "text-emerald-600" : "text-indigo-600"} text-sm mt-1`}
                                >
                                  {t("Relative Time Cost:")}{" "}
                                  <span className="font-mono">
                                    +
                                    {selectedStrategy!.totalRaceTime.toFixed(1)}{" "}
                                    {t("sec")}
                                  </span>
                                </p>
                              </div>
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className="p-2 bg-white/50 dark:bg-slate-800 hover:bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 transition"
                                ...
                                <button
                                  ...
                                  className="p-2 bg-white/50 dark:bg-slate-800 hover:bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 transition"
                                  title={t("Download Summary")}
                                >
                                  <Download className="w-4 h-4" />
                                </button>
                                {selectedStratIndex === 0 ? (
                                  <div className="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 rounded-full text-sm font-semibold border dark:border-slate-700 border-emerald-200">
                                    {t("Fastest (Recommended)")}
                                  </div>
                                ) : (
                                  <div className="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-300 rounded-full text-sm font-semibold border dark:border-slate-700 border-indigo-200">
                                    {t("Inspecting")}
                                  </div>
                                )}
                                {selectedStrategy!._isManualValid !== true &&
                                  selectedStrategy!._isManualValid !==
                                    undefined && (
                                    <div className="px-3 py-1 bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300 rounded-full text-sm font-semibold border border-red-200 ml-2 animate-pulse">
                                      {t(
                                        selectedStrategy!
                                          ._isManualValid as string,
                                      )}
                                    </div>
                                  )}
                              </div>
                            ...
                            {selectedStrategy!.stints.some(
                              ...
                              <div className="bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-400 p-3 rounded-lg mb-5 flex items-center gap-2 text-sm font-bold">
                                <Siren className="w-5 h-5" />
                                {t(
                                  "EXTREME DNF RISK: Wait! One or more stints exceed 90% wear. Reduce laps to prevent parts wear-out.",
                                )}
                              </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
                              {selectedStrategy!.stints.map((stint, idx) => (
                                <div
                                  key={idx}
                                  className={`bg-slate-50 dark:bg-slate-950 rounded-xl p-4 border shadow-sm flex flex-col justify-between ${selectedStratIndex === 0 ? "border-emerald-100 dark:border-emerald-900/30" : "border-indigo-100 dark:border-indigo-900/30"} ${stint.wearEnd > 95 ? "border-red-300 dark:border-red-800" : ""}`}
                                >
                                  <div
                                    className={`text-xs font-bold uppercase mb-3 border-b dark:border-slate-700 pb-2 flex justify-between items-center ${selectedStratIndex === 0 ? "text-emerald-600 border-emerald-50" : "text-indigo-600 border-indigo-50"}`}
                                  >
                                    <span>
                                      {t("Stint")} {idx + 1}
                                    </span>
                                    <span className="font-mono opacity-50 px-1 bg-slate-50 dark:bg-slate-900 rounded">
                                      #{idx + 1}
                                    </span>
                                  </div>
                                  <div className="font-mono text-xl font-black text-slate-800 dark:text-slate-200 mb-3 mt-1 flex items-center justify-between">
                                    <div>
                                      <span className="text-slate-400 font-medium text-lg mr-1 ml-0.5">
                                        |{" "}
                                      </span>
                                      {COMPOUND_FULL_NAMES[stint.tyres]}
                                    </div>
                                    {stint.conditionsPenalty !== undefined &&
                                      stint.conditionsPenalty > 0 && (
                                        <div className="-mt-1 px-1 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 text-[10px] uppercase font-sans antialiased tracking-wide rounded flex items-center gap-1 cursor-help">
                                          ...
                                          Cost
                                          <Tooltip
                                            content={`Time penalty due to weather: +${stint.conditionsPenalty.toFixed(1)}s`}
                                          />
                                        </div>
                                      )}
                                  </div>
                                  <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1 bg-slate-50 dark:bg-slate-900 p-2 rounded-lg">
                                    <div className="mb-2">
                                      ...
                                      {manualStintLaps &&
                                        manualStintLaps.length > 1 &&
                                        idx < manualStintLaps.length - 1 && (
                                          <input
                                            type="range"
                                            min="1"
                                            max={
                                              manualStintLaps[idx] +
                                              manualStintLaps[idx + 1] -
                                              1
                                            }
                                    ...
                                    <p className="flex justify-between items-center text-xs">
                                      {t("Wear:")}{" "}
                                      <strong
                                        className={`${stint.wearEnd > 95 ? "text-red-500" : "text-slate-800 dark:text-slate-200"}`}
                                      >
                                        {stint.wearEnd.toFixed(1)}%
                                      </strong>
                                    </p>
                                    <p className="flex justify-between items-center text-xs">
                                      {t("Fuel:")}{" "}
                                      <strong className="text-slate-800 dark:text-slate-200">
                                        {Math.ceil(stint.fuelStart)} L
                                      </strong>
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div>
                                  <div className="flex items-center gap-2">
                                    <h4 className="text-sm font-bold text-indigo-900 mb-1">
                                      {t("Strategy Engine Recommendation")}
                                    </h4>
                                    <span className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-2 py-[2px] rounded text-[9px] font-bold uppercase tracking-wider mb-1">
                                      {t("AI Insight")}
                                    </span>
                                  </div>
                                  <p className="text-xs text-indigo-800/80 leading-relaxed">
                                    {t("In this")}{" "}
                                    {selectedStrategy!.stints.length - 1}-
                                    {t(
                                      "stop strategy, the predominant compound is",
                                    )}{" "}
                                    <strong className="text-indigo-900">
                                      {
                                        COMPOUND_FULL_NAMES[
                                          selectedStrategy!.stints[0].tyres
                                        ]
                                      }
                                    </strong>
                                    .
                                    {weather.tempMax > 25
                                      ? ` ${t("Hot weather")} (${weather.tempMax}°C) ${t("demands careful tyre management. Lower your aggression by")} ~${Math.min(20, Math.floor((weather.tempMax - 25) * 2))} ${t("points to avoid overheating.")}`
                                      : ` ${t("Temperatures are optimal")} (${weather.tempMax}°C), ${t("aggressive strategies can be executed safely.")}`}
                                    {player.riskAggression > 60
                                      ? ` ${t("Your high risk factor")} (${player.riskAggression}) ${t("brings short term pace (est.")} -0.${Math.floor(player.riskAggression / 10)}s ${t("per lap) but beware of unexpected tyre loss.")}`
                                      : ` ${t("Balanced driving")} (${player.riskAggression}) ${t("allows optimal tyre degradation, saving ~1 pit stop over hyper-aggressive drivers.")}`}
                                    {selectedStrategy!.stints[0].laps > 30 &&
                                      ` ${t("The long first stint of")} ${selectedStrategy!.stints[0].laps} ${t("laps means you will start with heavy fuel. Expect a pace deficit of")} ~1.2s ${t("in the first 5 laps compared to lighter cars.")}`}
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="md:col-span-2 flex flex-wrap items-center gap-3 mt-2 px-2">
                              ...
                              {selectedStrategy!.stints.map((stint, idx) => (
                                ...
                                >
                                  <div
                                    className={`w-4 h-4 rounded flex items-center justify-center border transition ${!hiddenStints.includes(idx) ? "bg-indigo-500 border-indigo-500" : "bg-transparent border-slate-300 dark:border-slate-600 dark:group-hover:border-slate-400"}`}
                                  >
                                    {!hiddenStints.includes(idx) && (
                                      <svg
                                  ...
                                  />
                                  <span
                                    className={`text-sm font-medium transition ${!hiddenStints.includes(idx) ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}
                                  >
                                    Stint {idx + 1}
                                  </span>
                                </label>
                            ...
                            <div className="mt-5">
                              ...
                              <div className="flex w-full h-8 gap-2 overflow-visible bg-slate-100 dark:bg-slate-900/50 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-inner">
                                {selectedStrategy?.stints.map((stint, idx) => {
                                  const totalStints =
                                    selectedStrategy.stints.length;
                                  const widthPct =
                                    (stint.laps /
                                      selectedStrategy.stints.reduce(
                                        (sum, s) => sum + s.laps,
                                        0,
                                      )) *
                                    100;
                        ...
                        <div>
                          <h4 className="text-md font-semibold text-slate-700 dark:text-slate-300 mb-4 px-1">
                            {t("All Valid Strategies")}
                          </h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5">
                            <AnimatePresence>
                              {results.slice(0, 12).map((strat, sIdx) => {
                                const diff =
                                  strat.totalRaceTime -
                                  bestStrategy!.totalRaceTime;
                                  ...
                                  >
                                    <div className="flex items-center justify-between mb-2">
                                      <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 text-xs">
                                        {strat.stops} Pit
                                        {isBest && (
                                          <span className="text-[8px] bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 px-1 py-0.5 rounded uppercase tracking-wider">
                                            {t("Best")}
                                          </span>
                                        )}
                                      </div>
                                      <div
                                        className={`text-[10px] font-mono font-semibold ${isBest ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}
                                      >
                                        {isBest
                                          ? "Ref"
                                          : `+${diff.toFixed(1)}s`}
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {strat.stints.map((st, idx) => (
                                        <div
                                          key={idx}
                                          className={`flex items-center text-[10px] ${isSelected ? "opacity-100" : "opacity-70"}`}
                                        >
                                          <span className="font-medium text-slate-600 dark:text-slate-300">
                                            {st.laps}v
                                          </span>
                                          <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400 ml-0.5">
                                            {COMPOUND_FULL_NAMES[
                                              st.tyres
                                            ].charAt(0)}
                                          </span>
              ...
              {["assistant", "rival_analysis", "telemetry"].includes(
                activeTab,
              ) && (
                <RivalAnalysisTab
                  t={t}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                {[...systemLogs, ...APP_NOTIFICATIONS][0]?.title}
              </p>
              <div className="flex gap-4 mt-3">
                <button
                  ...
                  className="text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 transition"
                >
                  {t("Mark as Read")}
                </button>
                <button
      ...
      {appView === "notifications" && (
        <div className="flex flex-col gap-6 animate-fade-in max-w-3xl mx-auto w-full">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 shadow-sm rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {[...systemLogs, ...APP_NOTIFICATIONS].filter(
                (n) => n.type === "in-game",
              ).length === 0 ? (
                ...
                [...systemLogs, ...APP_NOTIFICATIONS]
                  .filter((n) => n.type === "in-game")
                  .map((note) => (
                    <div
                      key={note.id}
                      className={`p-4 border rounded-xl ${isNotificationNew(note) ? "bg-indigo-50 dark:bg-indigo-900/30 border-indigo-100 dark:border-indigo-800" : "bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700"}`}
                    >
                      <div className="flex justify-between items-start">
                        <p className={`text-sm font-bold ${isNotificationNew(note) ? "text-indigo-800 dark:text-indigo-300" : "text-slate-800 dark:text-slate-300"}`}>
                          {t(note.title)}
                        </p>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => {
                              setLocalSettings(s => ({
                                ...s,
                                dismissedAlerts: [...(s.dismissedAlerts || []), note.id]
                              }));
                            }}
                            className="p-1 rounded opacity-50 hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/10 transition"
                            title={t("Dismiss")}
                          >
                            <Trash2 className="w-4 h-4 text-slate-500 dark:text-slate-400" />
                          </button>
                          <span className="text-[9px] uppercase tracking-wider font-bold bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded text-slate-600 dark:text-slate-400">
                            {t("Game")}
                          </span>
                        </div>
                      </div>
                      <p className={`text-xs mt-2 ${isNotificationNew(note) ? "text-indigo-600 dark:text-indigo-400" : "text-slate-600 dark:text-slate-400"}`}>
                        {t(note.message)}
                      </p>
                      <p className={`text-[10px] mt-3 font-mono ${isNotificationNew(note) ? "text-indigo-400 dark:text-indigo-500" : "text-slate-400 dark:text-slate-500"}`}>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 shadow-sm rounded-2xl p-6 shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
              ...
              {t("Update Notes History")}
            </h3>
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
              {[...systemLogs, ...APP_NOTIFICATIONS]
                .filter((n) => n.type === "patch-note")
                .map((note) => (
                  <div
                    key={note.id}
                    className={`p-4 border rounded-xl ${isNotificationNew(note) ? "bg-indigo-50 dark:bg-indigo-900/30 border-indigo-100 dark:border-indigo-800" : "bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700"}`}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-8 max-w-xl w-full border border-slate-200 dark:border-slate-800 space-y-6">
            ...
            <div className="text-center">
              ...
              >
                Welcome to Pitwall <span className="text-indigo-600">Pro</span>!
              </h2>
              <p className="mt-3 text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                To take full advantage of our strategy tools, please follow the
                initial setup steps.
              </p>
            </div>
            <div className="space-y-4">
              <div className="flex gap-4 items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                <div className="bg-blue-100 dark:bg-blue-900 text-blue-600 font-black w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                  1
                </div>
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    ...
                    <span className="font-bold text-indigo-500">Settings</span>{" "}
                    to auto-fetch Driver Data, Car Wear and Track Telemetry. You
                    can select exactly which data gets pulled.
                  </p>
                </div>
              </div>
              <div className="flex gap-4 items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Verify track conditions and setup weather configurations in
                    the{" "}
                    <span className="font-bold text-indigo-500">Assistant</span>{" "}
                    tab. The app predicts base pit times automatically!
                  </p>
                </div>
              </div>
              <div className="flex gap-4 items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Modify Laps and select tyre compounds in Strategy planner.
                    Compare fuel use instantly across strategy variants.
                  </p>
                </div>
              </div>
            </div>
            <div className="flex justify-center pt-4">
              <button
                onClick={() =>
                  setLocalSettings({ ...settings, onboardingSeen: true })
                }
