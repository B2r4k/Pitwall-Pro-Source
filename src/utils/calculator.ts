import { Track, TyreCompound, Weather, PlayerStats, StrategyResult, Stint } from '../types';
import { COMPOUND_PACE, COMPOUND_WEAR_RATE, COMPOUND_IDEAL_TEMP, SEVERITY_MULTIPLIERS } from '../data';
 ...
 */
export function calculateWearPerLap(compound: TyreCompound, params: CalcParams, rainProb: number = 0): number {
  ...
  if (compound === 'Rain') {
    if (rainProb < 10) rainWearMultiplier = 4.5; // Destroyed on dry
  ...
  const concentrationPenalty = Math.max(0, 250 - params.player.driverFocus) * 0.0001;
  const staminaPenalty = Math.max(0, 250 - params.player.driverStamina) * 0.0002;
  ...
  const driverStatsMultiplier = (1.0 + concentrationPenalty + staminaPenalty - expBonus) * weightWearMultiplier;

  // Car parts impact on wear
  let partsWearMultiplier = 1.0;
  if (params.player.pha && (params.player.pha.handling > 0 || params.player.pha.power > 0 || params.player.pha.acceleration > 0)) {
    ...
    const aeroBrakesCost = ((p.frontWing + p.rearWing + p.brakes) / 3 - 1) * 0.004; 
  ...
  return baseRate * trackMultiplier * playerMultiplier * tempMultiplier * aggressionMultiplier * driverStatsMultiplier * rainWearMultiplier * supplierWearMult * partsWearMultiplier * globalGproWearCalibrator;
}
...
export function calculateFuelPerLap(params: CalcParams): number {
  ...
  if (params.player.pha && (params.player.pha.power > 0 || params.player.pha.acceleration > 0)) {
     const pha = params.player.pha;
  ...
  return params.player.baseFuelPerLap * trackMultiplier * (partsFuelMultiplier + riskFuelCost);
}

/**
 * Estimates the percentage wear incurred by the car parts after a full race
 */
export function calculateRaceCarWear(params: CalcParams): Record<keyof PlayerStats['carParts'], number> {
  ...
  const trackMultiplier = SEVERITY_MULTIPLIERS[params.track.wearSeverity] || 1.0;
  
  // Helper to cap precision
  const format = (val: number) => parseFloat((val * riskMultiplier * distanceMultiplier * trackMultiplier).toFixed(1));
...
export function getStintRainProbability(startLap: number, endLap: number, totalLaps: number, rainProps: Weather['rainProps']): number {
  ...
  for (let lap = startLap; lap < endLap; lap++) {
    ...
    else if (fraction < 0.50) sum += rainProps.r2;
 ...
 */
export function simulateStint(laps: number, compound: TyreCompound, params: CalcParams, startLap: number = 0): { valid: boolean, wearEnd: number, timeCost: number, fuelNeeded: number, conditionsPenalty: number } {
  const rainProb = getStintRainProbability(startLap, startLap + laps, params.track.laps, params.weather.rainProps);
  ...
  return { valid: validWear && validFuel, wearEnd: totalWear, timeCost, fuelNeeded, conditionsPenalty: conditionsPacePenalty };
}
...
export function analyzeStrategies(params: CalcParams): StrategyResult[] {
  ...
  for (let stops = 0; stops <= maxStops; stops++) {
    ...
    const stintLapsArray = Array.from({length: numStints}, (_, i) => baseLaps + (i < extraLaps ? 1 : 0));
    ...
    for (const dry of dryCompounds) {
      ...
      const compoundArrays = Array.from({length: numStints}, () => allowedCompounds);
    ...
    const uniqueStrings = Array.from(new Set(allCombinations.map(c => c.join('-'))));
    const uniqueCombinations = uniqueStrings.map(str => str.split('-') as TyreCompound[]);
    ...
    for (const combo of uniqueCombinations) {
      ...
      if (validStrategy && stints.length === numStints) {
        ...
        for (let i = 0; i < stops; i++) {
          ...
          const pitTime = params.track.pitTimeBase + (nextStintFuel / params.pitRefuelRate) + trafficPenaltyPerStop;
...
export function calculateBaseFuel(
  ...
  const track = trackDatabase.find(t => t.id === trackId) || trackDatabase.find(t => t.id === 'custom')!;
...
export function calculateBaseWearMultiplier(
  ...
  const knownFactors = baseRate * trackMultiplier * tempMultiplier * aggressionMultiplier * partsWearMultiplier * driverStatsMultiplier * supplierWearMult * globalGproWearCalibrator;
